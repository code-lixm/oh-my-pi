...
struct PlaybackContext {
	fill:                    PlaybackFill,
	stop_signal:             Arc<dyn PlaybackStopSignal>,
	stopped:                 Arc<AtomicBool>,
	callback_thread:         Arc<AtomicUsize>,
	callback_enqueue_status: Arc<AtomicI32>,
}
...
// The callback must not allocate or unwind through AudioQueue's C boundary.
// Retaining the first failure lets the synchronous stop path report it after
// the queue has finished all in-flight callbacks.
fn record_callback_enqueue_failure(
	stopped: &AtomicBool,
	callback_enqueue_status: &AtomicI32,
	status: i32,
) -> bool {
	if stopped
		...
		.is_ok()
	{
		callback_enqueue_status.store(status, Ordering::Release);
		true
	} else {
		false
	}
...
fn callback_enqueue_result(
	callback_enqueue_status: &AtomicI32,
	direction: &str,
	cleanup: VoiceResult<()>,
) -> VoiceResult<()> {
	let status = callback_enqueue_status.load(Ordering::Acquire);
	if status == 0 {
		return cleanup;
	}
	match cleanup {
		Ok(()) => {
			Err(format!("CoreAudio {direction} callback buffer re-enqueue failed (OSStatus {status})"))
		},
		Err(cleanup_error) => Err(format!(
			"CoreAudio {direction} callback buffer re-enqueue failed (OSStatus {status}); \
			 {cleanup_error}"
		)),
	}
}
...
unsafe extern "C" fn playback_callback(
	...
	if !unsafe { (*context).stopped.load(Ordering::Acquire) } {
		...
		if status != 0 {
			// SAFETY: The context remains live until synchronous queue disposal completes.
			if unsafe {
				record_callback_enqueue_failure(
					&(*context).stopped,
					&(*context).callback_enqueue_status,
					status,
				)
			} {
				// SAFETY: The context remains live until synchronous queue disposal completes;
				// the signal's unsafe contract prevents unwinding through AudioQueue.
				unsafe { (*context).stop_signal.mark_stopped() };
			}
		}
	}
	// SAFETY: The context remains live until synchronous queue disposal completes.
...
impl PlaybackDevice {
	...
	pub fn start(
		...
		let callback_enqueue_status = Arc::new(AtomicI32::new(0));
		let mut context = Box::new(PlaybackContext {
			fill,
			stop_signal,
			stopped: Arc::clone(&stopped),
			callback_thread: Arc::clone(&callback_thread),
			callback_enqueue_status: Arc::clone(&callback_enqueue_status),
		});
	...
	pub fn stop(&mut self) -> VoiceResult<()> {
		...
		let Some(queue) = self.queue.take() else {
			return callback_enqueue_result(&self.callback_enqueue_status, "playback", Ok(()));
		};
		...
		let cleanup = queue.stop_and_dispose();
		drop(context);
		callback_enqueue_result(&self.callback_enqueue_status, "playback", cleanup)
	}
}
...
impl CaptureDevice {
	...
	pub fn stop(&mut self) -> VoiceResult<()> {
		self.stopped.store(true, Ordering::Release);
		let Some(queue) = self.queue.take() else {
			return callback_enqueue_result(&self.callback_enqueue_status, "capture", Ok(()));
		};
		let Some(context) = self.context.take() else {
			self.queue = Some(queue);
		...
		let cleanup = queue.stop_and_dispose();
		drop(context);
		callback_enqueue_result(&self.callback_enqueue_status, "capture", cleanup)
	}
}
...
mod tests {
	...
	#[test]
	fn callback_failure_keeps_first_os_status() {
		let stopped = AtomicBool::new(false);
		let callback_enqueue_status = AtomicI32::new(0);

		record_callback_enqueue_failure(&stopped, &callback_enqueue_status, -7001);
		record_callback_enqueue_failure(&stopped, &callback_enqueue_status, -7002);

		assert!(stopped.load(Ordering::Acquire));
		assert_eq!(callback_enqueue_status.load(Ordering::Acquire), -7001);
	}

	#[test]
	fn externally_stopped_callback_does_not_report_enqueue_failure() {
		let stopped = AtomicBool::new(true);
		let callback_enqueue_status = AtomicI32::new(0);

		record_callback_enqueue_failure(&stopped, &callback_enqueue_status, -7003);

		assert_eq!(callback_enqueue_status.load(Ordering::Acquire), 0);
		assert_eq!(callback_enqueue_result(&callback_enqueue_status, "capture", Ok(())), Ok(()));
	}

	#[test]
	fn callback_failure_is_reported_after_successful_cleanup() {
		let callback_enqueue_status = AtomicI32::new(-7004);

		assert_eq!(
			callback_enqueue_result(&callback_enqueue_status, "playback", Ok(())),
			Err("CoreAudio playback callback buffer re-enqueue failed (OSStatus -7004)".to_owned())
		);
	}

	#[test]
	fn callback_failure_preserves_cleanup_error_detail() {
		let callback_enqueue_status = AtomicI32::new(-7005);

		assert_eq!(
			callback_enqueue_result(
				&callback_enqueue_status,
				"capture",
				Err("CoreAudio queue dispose failed (OSStatus -50)".to_owned()),
			),
			Err(
				"CoreAudio capture callback buffer re-enqueue failed (OSStatus -7005); CoreAudio \
				 queue dispose failed (OSStatus -50)"
					.to_owned()
			)
		);
	}
