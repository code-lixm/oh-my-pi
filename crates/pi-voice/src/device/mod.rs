...
/// Callback-safe bridge from a playback backend to its engine state.
///
/// # Safety
/// Implementations MUST ensure [`Self::mark_stopped`] never panics or blocks,
/// because `CoreAudio` invokes it from an `extern "C"` callback.
pub(crate) unsafe trait PlaybackStopSignal: Send + Sync {
...
impl PlaybackDevice {
	...
	pub fn start(
		...
		Ok(Self { inner: imp::PlaybackDevice::start(config, fill, stop_signal)? })
	}
