...
use crate::{
	VoiceResult,
	device::{CaptureDevice, DeviceConfig, PlaybackDevice, PlaybackStopSignal},
};
...
// SAFETY: `mark_stopped` only updates atomics and wakes existing waiters; it
// does not unwind or block when called by a device callback.
unsafe impl PlaybackStopSignal for PlaybackState {
	fn mark_stopped(&self) {
		Self::mark_stopped(self);
	}
}
...
impl PlaybackStream {
	...
	pub fn start(sample_rate: u32) -> VoiceResult<Self> {
		...
		let stop_signal: Arc<dyn PlaybackStopSignal> = state.clone();
