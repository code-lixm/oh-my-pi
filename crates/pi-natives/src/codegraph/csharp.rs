...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_ASYNC, FLAG_IS_EXPORTED, FLAG_IS_STATIC,
		FUNCTION_REF_CODE, NONE, NONE_STR, NodeRow, RefRow, StrRef, Tables, build_meta,
		edge_kind_index, node_kind_index,
	},
...
/// JS `\s` (WhiteSpace ∪ LineTerminator) — differs from Rust's
/// `\p{White_Space}` on U+FEFF (JS: yes) and U+0085 (JS: no). The chained-call
/// inner-callee strip (`.replace(/\s+/g, '')`) runs on arbitrary source slices,
/// so match JS exactly.
