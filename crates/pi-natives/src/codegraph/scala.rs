...
impl<'t> Walker<'t> {
	...
	fn hook(&mut self, node: Node<'t>) -> bool {
		match node.kind() {
			"val_definition" | "var_definition" => {
				...
				let created = self.create_node(kind, &name, node, Extra {
					signature,
					visibility,
					..Default::default()
				});
	...
	fn extract_method_or_function(&mut self, node: Node<'t>) {
		...
		let row =
			self.create_node(if is_method { "method" } else { "function" }, &name, node, Extra {
				docstring,
				signature,
				visibility,
				is_async: Some(false),
		...
		self
			.stack
			.push(Scope { row, kind: if is_method { "method" } else { "function" }, name });
		if let Some(body) = node.child_by_field_name("body") {
			self.visit_body(body);
		}
		self.stack.pop();
	...
	fn extract_class(&mut self, node: Node<'t>, kind: &'static str) {
		...
		let visibility = self.visibility_of(node);
		let row =
			self.create_node(kind, &name, node, Extra { docstring, visibility, ..Default::default() });
		let Some(row) = row else { return };
	...
	fn extract_enum(&mut self, node: Node<'t>) {
		...
		let docstring = preceding_docstring(node, self.src);
		let visibility = self.visibility_of(node);
		let row = self.create_node("enum", &name, node, Extra {
			docstring,
			visibility,
			..Default::default()
		});
	...
	fn extract_import(&mut self, node: Node<'t>) {
		...
		let created = self.create_node("import", &module, node, Extra {
