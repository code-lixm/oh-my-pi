...
use super::{
	Extra, Scope, Walker, body_of, is_builtin_type, is_literal_receiver, is_react_hoc,
	is_variable_type, is_vue_collection_name,
};
use crate::{buffers::edge_kind_index, textutil as util};
...
impl<'t> Walker<'t> {
	...
	fn extract_react_component_node(
		...
		self
			.stack
			.push(Scope { row, kind: "component", name: name.to_string() });
	...
	pub(super) fn extract_property(&mut self, node: Node<'t>) -> Option<(u32, String)> {
		...
		let row = self.create_node("property", &name, node, Extra {
			docstring,
	...
	pub(super) fn extract_variable(&mut self, node: Node<'t>) {
		...
		for i in 0..node.named_child_count() {
			...
			if let Some(v) = value {
				if util::pascal_case().is_match(&name) {
					if let Some(inner) = self.react_component_hoc(v) {
						self.extract_react_component_node(&name, child, inner, Extra {
			...
			let var_row = self.create_node(kind, &name, child, Extra {
				docstring: docstring.clone(),
				signature: init_signature.clone(),
				is_exported: Some(is_exported),
				..Extra::default()
	...
	fn extract_rtk_hook_bindings(&mut self, pattern: Node<'t>, is_exported: bool) {
		for i in 0..pattern.named_child_count() {
			...
			self.create_node("function", &name, binding, Extra {
				is_exported: Some(is_exported),
	...
	fn extract_rtk_endpoints(&mut self, obj: Node<'t>) {
		for i in 0..obj.named_child_count() {
			...
			if let Some(handler) = self.rtk_endpoint_handler(value) {
				...
				let row = self.create_node("function", &key_name, value, Extra {
					signature: Some(sig),
					..Extra::default()
				});
				if let Some(row) = row {
					self
						.stack
						.push(Scope { row, kind: "function", name: key_name });
	...
	fn extract_ts_type_alias_members(&mut self, value: Node<'t>, alias_row: u32, alias_name: &str) {
		...
		self
			.stack
			.push(Scope { row: alias_row, kind: "type_alias", name: alias_name.to_string() });
		for obj_type in object_types {
			for i in 0..obj_type.named_child_count() {
				...
				let member_kind: &'static str =
					if child.kind() == "method_signature" || self.is_ts_function_typed_property(child) {
						"method"
					} else {
						"property"
					};
	...
	pub(super) fn extract_decorators_for(&mut self, decl: Node<'t>, decorated_row: u32) {
		...
		// 2. Preceding siblings (TypeScript class style), stopping at the first
		//    non-decorator so an earlier declaration's decorators never leak in.
		//    Matching by startIndex, not object identity.
