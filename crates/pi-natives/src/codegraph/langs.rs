...
// see grammars/kotlin and the kotlin checklist §Grammar prep). Symbols are
// prefixed at build time so this addon can coexist with pi-ast's own grammar
// set inside the same cdylib.
unsafe extern "C" {
	fn codegraph_tree_sitter_kotlin() -> *const ();
}
...
unsafe extern "C" {
	fn codegraph_tree_sitter_lua() -> *const ();
}
// Vendored scala grammar (build.rs-compiled C — the wasm is master@0aca5d0a6f,
// 30 states past the v0.26.0 crate; see grammars/scala and the scala
// checklist).
unsafe extern "C" {
	fn codegraph_tree_sitter_scala() -> *const ();
}
...
unsafe extern "C" {
	fn codegraph_tree_sitter_dart() -> *const ();
}
...
pub fn grammar_for(language: &str) -> Option<Language> {
	match language {
		...
		"kotlin" => Some(
			unsafe { tree_sitter_language::LanguageFn::from_raw(codegraph_tree_sitter_kotlin) }.into(),
		),
		...
		"lua" => Some(
			unsafe { tree_sitter_language::LanguageFn::from_raw(codegraph_tree_sitter_lua) }.into(),
		),
		...
		"scala" => Some(
			unsafe { tree_sitter_language::LanguageFn::from_raw(codegraph_tree_sitter_scala) }.into(),
		),
		...
		"dart" => Some(
			unsafe { tree_sitter_language::LanguageFn::from_raw(codegraph_tree_sitter_dart) }.into(),
		),
		_ => None,
	}
}
