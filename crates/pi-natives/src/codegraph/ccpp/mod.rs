...
//!    child is namespace_identifier/template_type/…, never a plain identifier,
//!    so it can't emit.
...
//!  - fn-ref capture (#756): cFamilySpec for both; cpp adds addressOfOnly (bare
//!    identifiers only qualify in file-scope value/list positions).
//!
...
/// normalizeValue's qualified `&Cls::m` member-pointer test
/// (`/^[A-Za-z_][\w:]*$/`).
...
impl<'t> Walker<'t> {
	...
	fn extract_type_alias(&mut self, node: Node<'t>) -> bool {
		...
		if resolved == Some("struct") {
			let Some(row) =
				self.create_node("struct", &name, node, Extra { docstring, ..Extra::default() })
			else {
				return true;
			};
			self.stack.push(Scope { row, kind: "struct", name });
		...
		if resolved == Some("enum") {
			let Some(row) =
				self.create_node("enum", &name, node, Extra { docstring, ..Extra::default() })
			else {
				return true;
			};
			self.stack.push(Scope { row, kind: "enum", name });
	...
	fn extract_import(&mut self, node: Node<'t>) {
		...
		let module_name: Option<String> =
			if let Some(sys) = self.find_child_by_kind(node, "system_lib_string") {
				...
				self
					.find_child_by_kind(lit, "string_content")
		...
		self.create_node("import", &module_name, node, Extra {
	...
	fn is_cpp_stack_construction(&self, node: Node) -> bool {
		...
		if !matches!(type_node.kind(), "type_identifier" | "template_type" | "qualified_identifier") {
			return false;
		}
		for i in 0..node.named_child_count() {
			let Some(child) = node.named_child(i) else {
	...
	fn extract_inheritance(&mut self, node: Node<'t>, class_row: u32) {
		...
		for i in 0..node.named_child_count() {
			...
			match child.kind() {
				"base_class_clause" => {
					for j in 0..child.named_child_count() {
						let Some(t) = child.named_child(j) else {
							continue;
						};
						if matches!(
							t.kind(),
