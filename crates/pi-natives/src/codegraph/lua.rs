...
//! The authoritative quirk list is
//! docs/design/lua-luau-kernel-port-checklist.md. Load-bearing oddities
//! preserved on purpose: the require/visitNode-hook ASYMMETRIES (top-level
//! requires — including inside top-level if/for/while blocks — mint import
//! nodes, while the identical statement in a function body emits `calls
//! "require"`; a top-level `local x = foo()` initializer emits NO calls ref
//! while a top-level global `x = foo()` does), the BFS string-win inside
//! require args (`require(script:WaitForChild("Kid"))` → import "Kid";
//! `require("a".."b")` → import "a"), raw-text callees verbatim (colon forms
//! `M:render` with `self` never stripped, brackets `t2[k2]`, newline-glued
//! chains byte-verbatim, the `(handler)` paren-conversion), receiver-QN methods
//! (`M.sub.deep::chained`, stack-QN nested globals like
...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_EXPORTED, FUNCTION_REF_CODE, NONE, NONE_STR,
		NodeRow, RefRow, StrRef, Tables, build_meta, edge_kind_index, node_kind_index,
	},
...
impl<'t> Walker<'t> {
	...
	fn emit_require(&mut self, call: Node<'t>, module: &str) {
		...
		let imp = self
			.create_node("import", module, call, Extra { signature: Some(sig), ..Default::default() });
	...
	fn extract_function(&mut self, node: Node<'t>) {
		...
		let fn_row = self.create_node("function", &name, node, Extra {
			docstring,
			signature,
			is_exported,
			..Default::default()
	...
	fn extract_method(&mut self, node: Node<'t>, receiver: String) {
		...
		let method_row = self.create_node("method", &name, node, Extra {
			docstring,
			signature,
