...
impl<'t> Walker<'t> {
	...
	fn extract_type_alias(&mut self, node: Node<'t>) -> bool {
		...
		if resolved == Some("struct_type") {
			let Some(row) = self.create_node("struct", &name, node, Extra {
				docstring,
				is_exported,
				..Extra::default()
			}) else {
		...
		if resolved == Some("interface_type") {
			let Some(row) = self.create_node("interface", &name, node, Extra {
				docstring,
				is_exported,
				..Extra::default()
			}) else {
	...
	fn extract_go_interface_methods(
		...
		self
			.stack
			.push(Scope { row: iface_row, kind: "interface", name: iface_name.to_string() });
		...
		if node.kind() == "short_var_declaration" {
			...
			if let Some(left) = left {
				...
				for id in identifiers {
					...
					self.create_node("variable", &name, node, Extra {
	...
	fn extract_import(&mut self, node: Node<'t>) {
		...
		let handle_spec = |w: &mut Self, spec: Node<'t>| {
			...
			w.create_node("import", &import_path, spec, Extra {
					...
					if let Some(r) = receiver {
						match r.kind() {
							...
							"call_expression" => {
								...
								let reencode = inner_fn.map(|f| f.kind() == "identifier").unwrap_or(false);
								if reencode {
	...
	fn flush_value_refs(&mut self, root: Node<'t>) {
		...
		while let Some(n) = dstack.pop() {
			...
			match n.kind() {
				"const_spec" | "var_spec" => {
					bump(&mut decl_counts, n.named_child(0), self.src, &targets)
				},
