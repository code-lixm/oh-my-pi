...
/// FIELD_ASSIGN_RE:
/// /(\w+)\s*(?:->|\.)\s*(\w+)\s*=\s*(\w+)\s*(?:->|\.)\s*(\w+)/g Pairs collected
/// as `lfield\0rfield`. Every byte position is a candidate start (JS advances
/// one unit on failure — suffix starts included); matches resume at their end.
