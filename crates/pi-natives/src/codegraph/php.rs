...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_EXPORTED, FLAG_IS_STATIC, FUNCTION_REF_CODE,
		NONE, NONE_STR, NodeRow, REF_FLAG_FILE_PATH, RefRow, StrRef, Tables, build_meta,
...
impl<'t> Walker<'t> {
	...
	fn extract_field(&mut self, node: Node<'t>) {
		...
		for elem in prop_elements {
			...
			self.create_node("field", &name, elem, Extra {
		...
		if let (Some(ns_prefix), Some(use_group)) = (ns_prefix, use_group) {
			...
			for clause in clauses {
				...
				if let Some(name) = name {
					...
					self.create_node("import", &full, node, Extra {
