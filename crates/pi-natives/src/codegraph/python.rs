...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_ASYNC, FLAG_IS_STATIC, FUNCTION_REF_CODE, NONE,
		...
		for i in 0..node.named_child_count() {
			...
			if child.kind() == "argument_list" {
				for j in 0..child.named_child_count() {
					let Some(arg) = child.named_child(j) else {
						continue;
					};
	...
	fn extract_import(&mut self, node: Node<'t>) {
		...
		for i in 0..node.named_child_count() {
			...
			if child.kind() == "dotted_name" {
				...
				self.create_node("import", &name, node, Extra {
		...
		for v in values {
			let (name, anchor) = match v.kind() {
				...
				"attribute" => {
					...
					match (obj, attr) {
						(Some(o), Some(a)) if o.kind() == "identifier" && self.text(o) == "self" => {
