...
use crate::{
	buffers::{
		...
		NodeRow, REF_FLAG_FILE_PATH, RefRow, StrRef, Tables, build_meta, edge_kind_index,
		node_kind_index,
	},
...
impl<'t> Walker<'t> {
		...
		if kind == "call" && node.child_by_field_name("receiver").is_none() {
			if let Some(method) = node.child_by_field_name("method") {
				if matches!(self.text(method), "include" | "extend" | "prepend") {
					...
					// no args node → hook declines (falls to extractImport →
					// nothing)
				}
			}
		}
	...
	fn extract_function(&mut self, node: Node<'t>) {
		...
		self.stack.pop();
		// `parameters` are NEVER walked — a call in a default value emits
		// nothing.
	}
