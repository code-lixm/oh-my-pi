...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_STATIC, FUNCTION_REF_CODE, NONE, NONE_STR,
...
impl<'t> Walker<'t> {
	...
	fn create_node(
		...
		self
			.nodes_meta
			.push(NodeMeta { kind, name: name.to_string(), qualified_name: qualified });
	...
	fn extract_field(&mut self, node: Node<'t>) {
		...
		if !declarators.is_empty() {
			...
			for decl in declarators {
				...
				let row = self.create_node(field_kind, &name, decl, Extra {
			...
			if let Some(name_node) = name_node {
				let name = self.text(name_node).to_string();
				self.create_node(field_kind, &name, node, Extra {
					docstring,
					visibility,
					is_static,
					..Extra::default()
	...
	fn extract_call(&mut self, node: Node<'t>) {
		...
		if let (Some(name_field), Some(object_field)) = (name_field, object_field) {
			...
			let receiver_name = if object_field.kind() == "field_access" {
				...
				match (inner, fld) {
					(Some(inner), Some(fld)) if matches!(inner.kind(), "this" | "this_expression") => {
	...
	fn extract_anonymous_class(&mut self, node: Node<'t>, body: Node<'t>) {
		...
		self
			.stack
			.push(Scope { row, kind: "class", name: anon_name });
		for i in 0..body.named_child_count() {
			if let Some(c) = body.named_child(i) {
				self.visit_node(c);
			}
			...
			match child.kind() {
				"superclass" | "extends_interfaces" => {
					...
					let targets: Vec<Node> = match type_list {
						Some(tl) => (0..tl.named_child_count())
							.filter_map(|j| tl.named_child(j))
							.collect(),
	...
	fn synthesize_lombok_members(&mut self, class_node: Node<'t>, class_row: u32) {
		...
		macro_rules! emit_method {
			($name:expr, $anchor:expr, $sig:expr, $from:expr, $is_static:expr, $ret:expr) => {{
				...
				if !name.is_empty() && !taken_methods.contains(&name) {
					...
					self.create_node("method", &name, $anchor, Extra {
						visibility:  Some(1),
						signature:   Some($sig),
		...
		if let Some(log_ann) = log_ann {
			if !taken_fields.contains("log") {
				self.create_node("field", "log", class_name_node, Extra {
					visibility: Some(2),
					is_static: Some(true),
