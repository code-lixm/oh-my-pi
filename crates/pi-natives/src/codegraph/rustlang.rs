...
//!   `contains` edge is a source-order name scan (an impl ABOVE its struct gets
//!   no edge). `impl Trait for Generic<T>`'s receiver resolves to the TRAIT
//!   (the only direct type_identifier), and methods get QN `Trait::method` —
//!   preserve, never "fix" via the grammar's trait:/type: fields.
//! - `const_item`/`static_item` ride the generic extractVariable fallback: kind
//!   is always `variable`, no signature, and EVERY direct `identifier` child
//!   mints a node (`const MAX: u32 = OTHER;` → two nodes, `MAX` + the phantom
//!   `OTHER`). Top-level initializer values are never body-walked.
...
//!   `self` receivers all collapse to the bare method name (`self` is node kind
//!   `self`, not `identifier`, so it dodges SKIP_RECEIVERS by falling through).
//!   Turbofish callees keep the raw `helper::<T>` text.
//! - `use` emits an import node named by the ROOT module (`crate`/`self`/…),
//!   one root `imports` ref, then one FULL-path `imports` ref per binding; `use
//!   x::*` (use_wildcard) emits nothing at all.
...
//! - A rust type alias emits NO ref to its aliased type (the shared code reads
//!   a `value` field; rust's field is `type`).
...
use crate::{
	buffers::{
		Arena, BoolFlags, EdgeRow, EmitOut, FLAG_IS_ASYNC, FLAG_IS_EXPORTED, FUNCTION_REF_CODE, NONE,
		NONE_STR, NodeRow, RefRow, StrRef, Tables, build_meta, edge_kind_index, node_kind_index,
	},
	docstring::preceding_docstring,
	ids, textutil as util,
};
...
pub fn extract(file_path: &str, source: &str) -> Result<EmitOut, String> {
	...
	w.nodes_meta
		.push(NodeMeta { kind: "file", name: base_name.to_string() });
	w.node_ids.push(ids::file_node_id(file_path));
	w.stack
		.push(Scope { row: 0, kind: "file", name: base_name.to_string() });
...
impl<'t> Walker<'t> {
	...
	fn create_node(
		...
		self
			.nodes_meta
			.push(NodeMeta { kind, name: name.to_string() });
		...
		if matches!(kind, "function" | "method" | "constant" | "variable") {
			self
				.value_scopes
				.push(ValueScope { row, node, name: name.to_string() });
		}
		Some(row)
	}
		...
		if as_method && !self.inside_class_like() {
			if let Some(receiver) = &receiver {
				let owner_row = self
					...
					.iter()
					.position(|m| {
						m.name == *receiver && matches!(m.kind, "struct" | "class" | "enum" | "trait")
					})
					.map(|i| i as u32);
	...
	fn extract_type_alias(&mut self, node: Node<'t>) {
		...
		let extra = Extra { docstring: preceding_docstring(node, self.src), ..Extra::default() };
	...
	fn extract_import(&mut self, node: Node<'t>) {
		let use_arg = (0..node.named_child_count())
			...
			.find(|c| {
				matches!(c.kind(), "scoped_use_list" | "scoped_identifier" | "use_list" | "identifier")
			});
		let Some(use_arg) = use_arg else { return };

		let module_name = self.root_module(use_arg);
		let signature = self.text(node).trim().to_string();
		self.create_node("import", &module_name.clone(), node, Extra {
			...
			match child.kind() {
				"trait_bounds" => {
					for j in 0..child.named_child_count() {
						let Some(bound) = child.named_child(j) else {
							continue;
						};
	...
	fn extract_rust_impl_item(&mut self, node: Node<'t>) {
		...
		let type_idents: Vec<Node> = (0..node.named_child_count())
			.filter_map(|i| node.named_child(i))
			.filter(|c| {
				matches!(c.kind(), "type_identifier" | "generic_type" | "scoped_type_identifier")
			})
			.collect();
	...
	fn flush_value_refs(&mut self, root: Node<'t>) {
		...
		let bump = |decl_counts: &mut HashMap<&'t str, u32>,
