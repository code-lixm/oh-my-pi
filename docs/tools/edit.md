...
| --- | --- | --- | --- |
| `input` | `string` | Yes | One or more file sections. Anchored sections must start with `[PATH#TAG]`; `TAG` is the four-hex snapshot tag emitted by the latest `read`/`grep`/current-disk `codegraph` source section/`write`/successful `edit`. Optional `*** Begin Patch` / `*** End Patch` envelope is ignored if present. |
...
Anchors come from `read`/`grep` or current-disk `codegraph` source output. `read` and CodeGraph emit a `[PATH#TAG]` header from the session snapshot store and original `LINE:TEXT` rows; copy the header into the edit section and only the line number into hunk headers. Do not re-run CodeGraph solely to refresh a post-edit tag.
...
- Missing tag for any section:
  - `Missing hashline snapshot tag for <path>; use \`[<path>#tag]\` from your latest read/search/CodeGraph source output. To create a new file, use the write tool.`
- Stray payload line:
