...
describe("/security", () => {
	...
	test("validate preserves the finding URI and selects the active prompt locale", async () => {
		const previousPromptLocale = getPromptLocale();
		try {
			setPromptLocale("en");
			const english = await command("validate secscan_fixture secf_fixture");

			setPromptLocale("zh-CN");
			const chinese = await command("validate secscan_fixture secf_fixture");

			expect(english).toEqual({
				prompt: expect.stringContaining(
					"Validate the security finding at `security://scans/secscan_fixture/findings/secf_fixture`",
				),
			});
			expect(chinese).toEqual({
				prompt: expect.stringContaining("验证安全发现 `security://scans/secscan_fixture/findings/secf_fixture`。"),
			});
		} finally {
