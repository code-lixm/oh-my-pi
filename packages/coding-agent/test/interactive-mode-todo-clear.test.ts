...
describe("InteractiveMode todo HUD persistence", () => {
	...
	async function replaceMode(todoClearDelay?: number, displayLanguage: "en" | "zh-CN" = "en"): Promise<void> {
		if (mode) {
			mode.stop();
		...
		session = new AgentSession({
			...
			settings: Settings.isolated({
				...(todoClearDelay === undefined ? {} : { "tasks.todoClearDelay": todoClearDelay }),
				displayLanguage,
			}),
			modelRegistry,
		});
	...
	async function createMode(todoClearDelay: number, displayLanguage: "en" | "zh-CN" = "en"): Promise<void> {
		...
		await replaceMode(todoClearDelay, displayLanguage);
	}
	...
	it("keeps closed todos while the list still has open work", () => {
		setTodoClearDelay(1);
		vi.useFakeTimers();
	...
	it("keeps the anchored todo panel in the live region while any todo is still open", () => {
		setTodoClearDelay(-1);
