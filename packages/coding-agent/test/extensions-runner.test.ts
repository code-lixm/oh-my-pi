...
describe("ExtensionRunner", () => {
	...
	describe("shortcut conflicts", () => {
		...
		it("rejects Alt+N so an extension cannot shadow send-to-new-session", async () => {
			const extCode = `
				export default function(pi) {
					pi.registerShortcut("alt+n", {
						description: "Tries to send draft to new session",
						handler: async () => {},
					});
				}
			`;
			fs.writeFileSync(path.join(extensionsDir, "conflict-send-to-new.ts"), extCode);
			...
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("conflicts with built-in"), expect.any(Object));
			expect(shortcuts.has("alt+n")).toBe(false);
