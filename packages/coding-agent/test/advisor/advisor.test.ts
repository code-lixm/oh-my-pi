...
describe("advisor", () => {
	...
	describe("AdviseTool", () => {
		...
		it("localizes advisor tool descriptions and results at runtime", async () => {
			...
			try {
				...
				setPromptLocale("en");
				const englishTool = new AdviseTool(() => {});
				expect(englishTool.description).toContain(
					"Send one concrete, terse piece of advice to the agent you are watching.",
				);
				const englishResult = await englishTool.execute("en-1", { note: "check the result" });
				expect(englishResult.content).toEqual([{ type: "text", text: "Recorded." }]);

				setSettingsUiLocale("zh-CN");
				setPromptLocale("zh-CN");
				const onAdvice = vi.fn();
				const chineseTool = new AdviseTool(onAdvice);
				expect(chineseTool.description).toContain("向你正在观察的代理提供一条具体、简洁的建议。");

				const recorded = await chineseTool.execute("zh-1", { note: "检查结果" });
				expect(recorded.content).toEqual([{ type: "text", text: "已记录。" }]);

				const duplicate = await chineseTool.execute("zh-2", { note: "检查结果" });
				expect(duplicate.content).toEqual([{ type: "text", text: "已忽略重复建议。" }]);
				expect(onAdvice).toHaveBeenCalledTimes(1);
			} finally {
	...
	describe("advice delivery policy", () => {
		...
		it("frames advisory output as guidance rather than an order", () => {
	...
	describe("createAdvisorMessageCard", () => {
		...
		it("renders the aggregated advisor header and every note", async () => {
			...
			expect(text).toContain("Advisor found 2 issues");
			expect(text).not.toContain("2 notes");
			expect(text).toContain("deleting the wrong file");
			expect(text).toContain("watch the empty case");
		});

		it("renders a full rounded advisor frame with an info title and body", async () => {
			const uiTheme = await getThemeByName("dark");
			if (!uiTheme) throw new Error("theme unavailable");
			...
			try {
				...
				const lines = createAdvisorMessageCard(
					{ notes: [{ note: "verify the frame body", severity: "concern" }] },
					() => true,
					uiTheme,
				).render(80);
				...
				expect(
					plainLines.some(
						line =>
							line.trimStart().startsWith(uiTheme.symbol("boxRound.topLeft")) &&
							line.trimEnd().endsWith(uiTheme.symbol("boxRound.topRight")),
					),
				).toBe(true);
				expect(
					plainLines.some(
						line =>
							line.trimStart().startsWith(uiTheme.symbol("boxRound.bottomLeft")) &&
							line.trimEnd().endsWith(uiTheme.symbol("boxRound.bottomRight")),
					),
				).toBe(true);
				expect(text).toContain("Advisor found 1 concern");
				expect(text).toContain("verify the frame body");
			} finally {
				setOutputBlockBorderStyle(previousBorderStyle);
			}
		});

		it("renders the Chinese severity summary and note text", async () => {
			const uiTheme = await getThemeByName("dark");
			if (!uiTheme) throw new Error("theme unavailable");
			...
			try {
				const card = createAdvisorMessageCard(
					{ notes: [{ note: "检查边界条件", severity: "blocker" }] },
					() => true,
					uiTheme,
				);
				const text = strip(card.render(80));
				expect(text).toContain("审阅助手发现 1 个阻断问题");
				expect(text).toContain("检查边界条件");
			} finally {
	...
	describe("AdvisorConfigOverlayComponent", () => {
		...
		it("renders Chinese advisor configuration chrome with explicit tools", async () => {
			...
			try {
				setThemeInstance(uiTheme);
				const overlay = make({
					advisors: [{ name: "Architecture", tools: ["read", "web_search"] }],
				});
				const text = strip(overlay.render(200));
				expect(text).toContain("审阅助手配置");
				expect(text).toContain("+ 添加审阅助手配置");
				expect(text).toContain("保存并应用");
				expect(text).toContain("read, web_search");
			} finally {
				setSettingsUiLocale(previousLocale);
			}
		});
