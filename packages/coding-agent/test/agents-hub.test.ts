...
describe("AgentsHub configuration strips", () => {
	...
	test("renders a YAML array model override as a readable dev model property", async () => {
		mockAgents();
		const settingsDir = await makeTempCwd();
		await fs.writeFile(
			path.join(settingsDir, "config.yml"),
			"task:\n  agentModelOverrides:\n    dev:\n      - anthropic/claude-sonnet-4-5\n      - openai/gpt-4.1-mini\n",
		);
		const settings = await Settings.loadReadOnly({ cwd: settingsDir, agentDir: settingsDir });
		const { strip } = await createHub(settings);

		expect(strip()).toContain("model: anthropic/claude-sonnet-4-5,openai/gpt-4.1-mini");
	});
