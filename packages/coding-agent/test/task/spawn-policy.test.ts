...
describe("bundled agent prompt scout gating", () => {
	it("renders Chinese /init without hard-coding an unavailable scout", () => {
		const previousPromptLocale = getPromptLocale();
		try {
			setPromptLocale("zh-CN");
			clearBundledCommandsCache();
			const init = loadBundledCommands().find(command => command.name === "init");
			if (!init) throw new Error("Bundled /init command was not loaded");

			expect(init.instructions.toLowerCase()).not.toContain("scout");
			expect(init.instructions).toMatch(/多个(?:研究|调研).*代理/);
			expect(init.instructions).toContain("`task`");
		} finally {
