...
describe("StatusLineComponent context breakdown", () => {
	...
	it("refreshes a rendered gauge when the context-line setting changes", () => {
		const { session } = makeSession({
			messages: [userMessage("hi"), assistantMessage("done")],
			usage: { tokens: 50_000, contextWindow: 100_000, percent: 50 },
		});
		...
		const off = comp.getTopBorder(80).content;
		...
		const percentage = comp.getTopBorder(80).content;

		expect(off).not.toContain(`${theme.getFgAnsi("border")}─`);
		expect(percentage).toContain(`${theme.getFgAnsi("border")}─`);
	});
