...
describe("auto thinking classifier helpers", () => {
	...
	it("exposes the Chinese max criteria only when the configured ceiling permits max", async () => {
		const previousLocale = getPromptLocale();
		try {
			setPromptLocale("zh-CN");
			const optedIn = createOnlineFixture(buildLadderModel("mock-zh-max", MAX_LADDER), "high", "max");
			await classifyDifficulty("migrate the production database without a reproducible rollback", optedIn.deps);
			const maxPrompt = optedIn.completeSimpleMock.mock.calls[0]?.[1] as { systemPrompt: string[] };

			expect(maxPrompt.systemPrompt[0]).toContain("你是一个用于 coding agent 的难度分类器。");
			expect(maxPrompt.systemPrompt[0]).toContain("`max`");
			expect(maxPrompt.systemPrompt[0]).toContain("没有可用复现");
			expect(maxPrompt.systemPrompt[0]).toContain("仅凭任务难度不足以选择 `max`");

			vi.restoreAllMocks();

			const xhigh = createOnlineFixture(buildLadderModel("mock-zh-xhigh", MAX_LADDER), "high", "xhigh");
			await classifyDifficulty("migrate the production database without a reproducible rollback", xhigh.deps);
			const xhighPrompt = xhigh.completeSimpleMock.mock.calls[0]?.[1] as { systemPrompt: string[] };

			expect(xhighPrompt.systemPrompt[0]).toContain("你是一个用于 coding agent 的难度分类器。");
			expect(xhighPrompt.systemPrompt[0]).toContain("`xhigh`");
			expect(xhighPrompt.systemPrompt[0]).not.toContain("`max`");
			expect(xhighPrompt.systemPrompt[0]).not.toContain("没有可用复现");
		} finally {
