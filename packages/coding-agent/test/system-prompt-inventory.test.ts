...
describe("system prompt tool inventory", () => {
	...
	it("conditionally includes the English read-only scout delegation gate", async () => {
		...
		expect(withScout).toMatch(/\bsingle\s+read[- ]only\s+scout\b/i);
		expect(withoutScout).not.toMatch(/\bread[- ]only\s+scout\b/i);
	});
