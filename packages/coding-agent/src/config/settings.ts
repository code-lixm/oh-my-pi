...
export class Settings {
	...
	async reloadForCwd(cwd: string): Promise<void> {
		...
		const prevCodeModeValues = CODE_MODE_SIGNAL_PATHS.map(path => this.get(path));
		this.#cwd = normalized;
		if (this.#persist) {
		...
		if (
			!Bun.deepEquals(
				CODE_MODE_SIGNAL_PATHS.map(path => this.get(path)),
				prevCodeModeValues,
			)
		) {
			codeModeSignal.fire();
	...
	#migrateRawSettings(raw: RawSettings): RawSettings {
		...
		if ("advisor.subagents" in raw) {
			const advisorObj = isRecord(raw.advisor) ? raw.advisor : {};
			if (!isRecord(raw.advisor)) raw.advisor = advisorObj;
