...
/** Keep the locale-selected model-prompt behavior while using the new shared workload templates. */
function localizedWorkloadPrompt(text: string): string {
	return selectPrompt(text, `${text}\n\n请使用中文回答。`);
}
	...
	switch (kind) {
		case "chat":
			return {
				...
				messages: user(
					opts.promptOverride ??
						localizedWorkloadPrompt(prompt.render(chatTemplate, { topic: pick(CHAT_TOPICS) }).trim()),
				),
			};
		case "generation":
			return {
				...
				messages: user(
					opts.promptOverride ??
						localizedWorkloadPrompt(prompt.render(generationTemplate, { topic: pick(GENERATION_TOPICS) }).trim()),
				),
			};
		case "prefill":
			return {
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	let board: ReturnType<typeof createLiveBoard> | undefined;
	...
	try {
		if (!json) {
			board = createLiveBoard(spinner => renderBenchProgress(progress, spinner), {
				isTTY: interactive,
				get columns() {
