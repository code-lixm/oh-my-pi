...
export async function runRenderCommand(args: RenderCommandArgs): Promise<number> {
	...
	try {
		...
		const sessionManager = await SessionManager.open(workingCopy, undefined, undefined, { suppressBreadcrumb: true });
		...
		for (let index = 0; index < (args.repaint ?? 0); index++) {
		...
		if (args.timing || args.repaint) {
			...
			if (repaints.length > 0) {
				const times = repaints.map(result => result.ms);
				const avg = times.reduce((left, right) => left + right, 0) / times.length;
