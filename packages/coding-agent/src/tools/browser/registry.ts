...
function browserKey(kind: BrowserKind, cwd: string): string {
	switch (kind.kind) {
		case "headless":
			return `headless:${kind.headless ? "1" : "0"}:${path.resolve(cwd)}`;
		case "spawned":
...
export async function acquireBrowser(kind: BrowserKind, opts: AcquireBrowserOptions): Promise<BrowserHandle> {
	const key = browserKey(kind, opts.cwd);
	for (;;) {
