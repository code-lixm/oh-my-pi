...
export interface ToolSession {
	...
	/** Look up an enabled tool through the eval bridge's permission gate. */
	getToolForEvalBridge?: (name: string) => AgentTool | undefined;
	/** Canonical tool names authorized through the eval bridge. */
	...
	/** Per-session semantic source coverage from CodeGraph, invalidated after file mutations. */
...
export const BUILTIN_TOOLS: Record<BuiltinToolName, ToolFactory> = {
	...
	next_step_offer: NextStepOfferTool.createIf,
};
...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "next_step_offer") return session.settings.get("communication.nextSteps") === "auto";
		if (name === "task") {
