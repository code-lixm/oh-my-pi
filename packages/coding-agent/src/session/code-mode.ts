...
/**
 * Synthetic eval bridge operations intercepted before the session registry.
 * A registered tool with one of these names is therefore callable only while
 * it remains on Code Mode's direct surface.
 */
export const CODE_MODE_EVAL_INTERNAL_OPS: Record<string, true> = {
	__agent__: true,
	__budget__: true,
	__completion__: true,
