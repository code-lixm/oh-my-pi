...
/**
 * Resolve every default session bucket that belongs to a cwd. The restored
 * cwd-derived directory remains canonical for new writes; the short-lived
 * develop hashed directory is read in place so an older live process can keep
 * writing through its original path while resume/continue still sees it.
 */
export function computeCompatibleSessionDirs(
	cwd: string,
	storage: SessionStorage,
	sessionsRoot: string = getSessionsDir(),
): string[] {
	const sessionDir = computeDefaultSessionDir(cwd, storage, sessionsRoot);
	const { hashedDirName } = getDefaultSessionDirName(cwd);
	const hashedDir = path.join(sessionsRoot, hashedDirName);
	return hashedDir !== sessionDir && storage.existsSync(hashedDir) ? [sessionDir, hashedDir] : [sessionDir];
}
