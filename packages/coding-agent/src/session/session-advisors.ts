...
export class SessionAdvisors {
	...
	#scheduleAdvisorFallbackProbe(advisor: ActiveAdvisor): void {
		if (getRetryFallbackRevertPolicy(this.#host.settings) !== "probe") return;
		if (
	...
	/**
	 * Apply the advisor fallback chain for a retryable provider failure.
	 * The `advisor` role hint lets an advisor-specific chain win over model/default mappings.
	 */
