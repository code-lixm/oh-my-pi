...
import xdevMountNoticePromptZh from "../prompts/system/xdev-mount-notice.zh-CN.md" with { type: "text" };
...
export interface SessionToolsHost {
	...
	setCodeModeNamespacesInfo?(info: ToolNamespacesInfo | undefined): void;
}
...
export class SessionTools {
	...
	/** Tools left directly model-visible by the last applied Code Mode partition. */
	...
	/** Reapply the preserved enabled set after model or Code Mode setting changes. */
	...
	takePendingXdevMountNotice(baseCatalogDelivered: boolean): CustomMessage<XdevMountNoticeDetails> | undefined {
		...
		return {
			...
			content: prompt.render(selectPrompt(xdevMountNoticePrompt, xdevMountNoticePromptZh), { added, removed, docs }),
	...
	refreshSkills(): Promise<void> {
		return this.runToolRegistryMutation(() => this.#refreshSkills());
	}
	...
	reconcileInspectImageAfterModelChange(): Promise<void> {
		return this.runToolRegistryMutation(async () => {
			...
			const model = this.#host.model();
			const modelName = model ? formatModelString(model) : tSettingsUi("the current model");
			this.#host.emitNotice(
				"info",
				after
	...
	#computeAppliedToolSignature(toolNames: string[], tools: AgentTool[], directToolNames?: readonly string[]): string {
		...
		return `${nameSegment}\u0003${descriptionSegment}\u0007${instructionsSegment}\u0008${mountedMCPRouteSegment}${directSegment}|${date}`;
	}
