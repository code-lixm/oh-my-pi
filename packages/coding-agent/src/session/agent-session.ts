...
import fallbackConnectivityProbe from "../prompts/system/fallback-connectivity-probe.md" with { type: "text" };
import fallbackConnectivityProbeZh from "../prompts/system/fallback-connectivity-probe.zh-CN.md" with { type: "text" };
...
export class AgentSession {
	...
	#updateActivityForAgentEvent(event: AgentEvent): void {
		switch (event.type) {
			...
			case "agent_end":
				...
				return;
			case "message_start":
				// An assistant envelope alone only proves that the provider accepted the
				// request. Claim "Thinking" only after a real thinking_start/delta;
				// text_start/delta similarly transitions to response streaming.
	...
	#beginInFlight(): void {
		...
		void this.#flushInFlightSettledCallbacks().finally(finish);
	}
	...
	constructor(config: AgentSessionConfig) {
		...
		const recoveryHost: TurnRecoveryHost = {
			...
			probeModelConnectivity: (model, signal) => this.probeModelConnectivity(model, signal),
		...
		const advisorsHost: SessionAdvisorsHost = {
			...
			clientBridge: () => this.#clientBridge,
			emitSessionEvent: event => this.#emitSessionEvent(event),
			probeModelConnectivity: (model, signal, credentialSessionId) =>
				this.probeModelConnectivity(model, signal, credentialSessionId),
			emitNotice: (level, message, source) => this.emitNotice(level, message, source),
	...
	/** Number of running background jobs visible through this session's snapshot scope. */
	getVisibleAsyncJobCount(): number {
		const manager = this.#asyncJobManager;
		if (!manager) return 0;
		return manager.getRunningJobCount(this.#agentId ? { ownerId: this.#agentId } : undefined);
	}
	...
	get runningAsyncJobCount(): number {
		...
		return this.#asyncJobManager.getRunningJobCount({ ownerId: this.#agentId });
	}
	...
	#processAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "agent_end") {
			...
			this.#advisors.onPrimaryAgentEnd(activeMessages);
			...
			if (activeGoal) {
				// Goal-mode payload rejections and replay-safe transient endpoint failures
				// consult the configured chain before maintenance can absorb the turn.
				const goalTransient =
					this.#recovery.isRetryableError(msg) && AIError.is(msg.errorId, AIError.Flag.Transient);
				if ((AIError.isPayloadRejection(msg) || goalTransient) && this.#recovery.isHardErrorFallbackEligible(msg)) {
				...
				if (compactionContinues || compactionResult.automaticContinuationBlocked) {
					// This early return bypasses the terminal error tail. Preserve a blocked
					// payload rejection so the transcript records why the goal stopped.
					if (
			...
			if (resumeResolvedStreamStall || this.#recovery.isRetryableError(msg)) {
				...
				// A chain-covered hard endpoint error follows the same configured retry
				// budget as transient errors before consulting another model.
				const didRetry = await this.#recovery.handleRetryableError(msg);
				if (didRetry) {
					await emitAgentEndNotification({ willContinue: true });
					return;
		...
		this.#schedulePostPromptTask(
			async signal => {
				...
				try {
					...
					await this.#runRunnableAgentTurn(() => this.agent.continue(signal), signal);
				} catch (error) {
	...
	async #emitSessionStopEvent(
		...
		const goalStatus = this.#goalModeState?.goal.status;
		const goalOwnsContinuation =
			this.#goalModeState?.enabled === true && (goalStatus === "active" || goalStatus === "budget-limited");
	...
	async resetSessionContext(): Promise<ResetSessionContextResult | undefined> {
		...
		// Context-file discovery has a process-lifetime cache. A reset must make
		// edited AGENTS.md and sibling rules visible to the next prompt.
		resetCapabilities();
	...
	/** Looks up an enabled eval-bridge tool with the session permission gate applied. */
	...
	/** Apply Code Mode to the initial tool surface before the first provider turn. */
	...
	/** Replace the Ctrl+P/`/models` cycle scope after provider discovery. */
	...
	async prompt(text: string, options?: PromptOptions): Promise<boolean> {
		// Manual compaction disconnects the agent subscription until its cleanup
		// reconnects and re-drains queued messages. Never dispatch a turn into that gap.
	...
	#scheduleQueuedMessageDrain(): void {
		...
		this.#scheduleAgentContinue({
			shouldContinue: () => {
				...
				return this.#canAutoContinueForFollowUp(true) && this.agent.hasQueuedMessages();
			},
			onSkip: () => {
				this.#queuedMessageDrainScheduled = false;
			},
	...
	async abort(options?: {
		...
		try {
			...
			if (options?.preserveCompaction) {
				...
				manualCompactionCleanup = this.abortCompaction(options?.reason);
			}
			this.abortBash();
			this.abortEval();
			const postPromptDrain = this.#cancelPostPromptTasks();
			...
			await this.agent.waitForIdle();
			// A replacement prompt must wait until manual compaction has restored its
			// subscription; otherwise its events are neither forwarded nor persisted.
	...
	async newSession(options?: NewSessionOptions): Promise<boolean> {
		...
		try {
			...
			// Drop the process-lifetime context-file cache before rebuilding so the
			// new transcript observes edits to AGENTS.md and related project rules.
			resetCapabilities();
	...
	/** Probe a model without touching transcript, agent state, or the active request lineage. */
	async probeModelConnectivity(
		model: Model,
		signal: AbortSignal,
		credentialSessionId = this.sessionId,
	): Promise<boolean> {
		const apiKey = await this.#modelRegistry.getApiKey(model, credentialSessionId, { signal });
		if (!apiKey) return false;
		const probeSessionId = `${credentialSessionId}:fallback-probe:${Snowflake.next()}`;
		const context: Context = {
			systemPrompt: [],
			messages: [
				{
					role: "user",
					content: selectPrompt(fallbackConnectivityProbe, fallbackConnectivityProbeZh),
					timestamp: Date.now(),
				},
			],
		};
		const options = this.#providerBoundary.prepareSimpleStreamOptions(
			{
				apiKey,
		...
		if (response.stopReason === "error" || response.stopReason === "aborted") return false;
		return response.content.some(block => block.type === "text" && block.text.trim().length > 0);
	}
	/**
