...
/** Accept the former direct AbortSignal argument while enabling streamed run events. */
export type CleanseSuiteRunInput = CleanseSuiteRunOptions | AbortSignal;
...
export interface CleanseDiagnosticSuite {
	...
	run(options?: CleanseSuiteRunInput): Promise<CleanseDiagnosticReport>;
}
...
function isAbortSignal(value: CleanseSuiteRunInput | undefined): value is AbortSignal {
	return (
		value !== undefined &&
...
function createSuite(
	...
	return {
		...
		async run(options?: CleanseSuiteRunInput): Promise<CleanseDiagnosticReport> {
			const normalized = isAbortSignal(options) ? { signal: options } : options;
			const { signal, events, flushMs = DEFAULT_FLUSH_MS } = normalized ?? {};
			const execute = async (
				plan: CheckerPlan,
...
async function runChecker(
	...
	const pollDelayMs = Number.isFinite(flushMs) && flushMs > 0 ? Math.trunc(flushMs) : DEFAULT_FLUSH_MS;
	...
	try {
		...
		const pollStop = Promise.withResolvers<void>();
		const waitForPoll = (): Promise<void> => {
			const { promise, resolve } = Promise.withResolvers<void>();
			const timer = setTimeout(resolve, pollDelayMs);
			timer.unref?.();
			return Promise.race([promise, pollStop.promise]).finally(() => clearTimeout(timer));
		};
		let running = true;
		...
		const poller =
			onDiagnostics && !plan.mutates
				...
				: undefined;
		const exited: Promise<{ exitCode: number | null; error?: unknown }> = child.exited
			.then(
				exitCode => ({ exitCode }),
				error => ({ exitCode: child.exitCode ?? null, error }),
			)
			.finally(() => {
				running = false;
				pollStop.resolve();
			});
		const [outcome] = await Promise.all([exited, pumps]);
		await poller;
		if (signal?.aborted) throw signal.reason ?? outcome.error ?? new Error("Checker interrupted");
		...
		if (outcome.exitCode !== 0 && parsedDiagnostics.length === 0) {
			diagnostics.push(checkerFailureDiagnostic(plan, outcome.exitCode, stdout.text, stderr.text));
		}
		emit(diagnostics);
		return result(outcome.exitCode, diagnostics);
	} catch (error) {
