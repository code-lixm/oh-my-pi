...
export async function runCleanseLoop(
	...
	const trySend = (entry: OwnerEntry): void => {
		...
		const send = dependencies
			...
			.then(delivered => {
				...
				if (delivered && !entry.released) {
					trySend(entry);
					return;
				}
				if (entry.released) {
					// Delivery can settle after the worker has exited; its chat can no
					// longer repair this batch, so give a fresh worker ownership instead.
					const touched = new Set<OwnerEntry>();
	...
	const takeBatches = (slots: number): CleanseFileIssues[][] => {
		const assignments = balanceDiagnostics([...pending.values()].flat(), slots);
		for (const assignment of assignments) {
			for (const group of assignment.groups) pending.delete(group.file ?? "");
		}
		return assignments.map(assignment => assignment.groups);
	};
	...
	const pump = (): void => {
		if (signal?.aborted || dispatchFailure || pending.size === 0) return;
		const slots = maxAgents - inFlight.size;
		if (slots <= 0) return;
		// Split everything currently known across the free slots with LPT's
		// deterministic least-loaded assignment; later arrivals form their own
		// balanced batch without moving a file away from its active owner.
		for (const groups of takeBatches(slots)) launch(groups);
	};
	...
	if (signal?.aborted) {
		return { status: "cancelled", workers: dispatched, waves: dispatched > 0 ? 1 : 0, report, outcomes };
	}
	if (dispatched === 0) {
		const status = report.diagnostics.length === 0 ? "clean" : "stalled";
		return { status, workers: 0, waves: 0, report, outcomes };
	}
	const verified = await dependencies.verify(signal);
	dependencies.onVerified?.(verified);
	if (signal?.aborted) {
		return { status: "cancelled", workers: dispatched, waves: 1, report: verified, outcomes };
	}
	return {
