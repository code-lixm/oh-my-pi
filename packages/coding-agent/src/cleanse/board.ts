...
export class CleanseBoardModel {
	...
	checkerFinished(check: CleanseCheckResult, durationMs: number): string {
		...
		const verdict =
			count === 0
				? chalk.green(tSettingsUi("clean"))
				: chalk.yellow(tSettingsUi("{count} issue{s}", { count, s: count === 1 ? "" : "s" }));
...
export function createCleanseStatusBoard(
	...
	return {
		...
		agentStarted(name, assignment) {
			if (!board.interactive) {
				const files = assignment.groups.map(group => group.file ?? tSettingsUi("<project>")).join(", ");
		...
		agentFinished(outcome, assignment) {
			if (!board.interactive) {
				...
				if (outcome.success) {
					...
					errors.write(
						`[fail] ${outcome.name}: ${oneLine(outcome.error ?? tSettingsUi("subagent failed"), ERROR_WIDTH)}\n`,
					);
				}
				return;
			}
...
function renderWaveHeader(
	...
	if (running > 0) parts.push(tSettingsUi("{count} running", { count: running }));
	if (tokens > 0) parts.push(tSettingsUi("{count} tok", { count: formatNumber(tokens) }));
	if (cost > 0) parts.push(formatCost(cost));
	parts.push(formatDuration(Date.now() - startedAt));
	return `${chalk.cyan(spinner)} ${tSettingsUi("Repairing")} [${bar}] ${parts.join(chalk.dim(" · "))}`;
}
...
function renderAgentRow(spinner: string, agentName: string, agent: RunningAgent): string {
	...
	if (toolCount > 0) meta.push(tSettingsUi("{count} tool{s}", { count: toolCount, s: toolCount === 1 ? "" : "s" }));
...
function renderOutcomeLine(
	...
	if (!outcome.success) {
		return `${chalk.red("✗")} ${outcome.name} ${files} ${chalk.red(oneLine(outcome.error ?? tSettingsUi("subagent failed"), ERROR_WIDTH))}`;
	}
	const meta: string[] = [];
	const toolCount = agent?.progress?.toolCount ?? 0;
	if (toolCount > 0) meta.push(tSettingsUi("{count} tool{s}", { count: toolCount, s: toolCount === 1 ? "" : "s" }));
	if (total && total.tokens > 0) meta.push(tSettingsUi("{count} tok", { count: formatNumber(total.tokens) }));
...
function agentActivity(progress: AgentProgress | undefined): string {
	if (!progress) return chalk.dim(tSettingsUi("starting"));
	if (progress.retryState) {
		return chalk.yellow(
			tSettingsUi("rate-limited · retry {attempt}/{maxAttempts}", {
	...
	return intent || chalk.dim(tSettingsUi("thinking"));
}

function compactFiles(assignment: CleanseAssignment): string {
	const files = assignment.groups.map(group => group.file ?? tSettingsUi("<project>"));
	const first = files[0] ?? tSettingsUi("<project>");
