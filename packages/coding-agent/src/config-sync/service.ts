...
import { getSyncConflictPath, loadSyncProfile, loadSyncState, requireSyncPassphrase, saveSyncState } from "./profile";
...
export async function getConfigSyncStatus(agentDir: string): Promise<SyncStatusResult> {
	const profile = await loadSyncProfile(agentDir);
	if (!profile) return { configured: false };
	const state = await loadSyncState(agentDir);
	const graph = await loadRemoteGraph(new S3ConfigSyncStore(profile));
	return {
		configured: true,
...
export async function synchronizeConfiguration(
	...
	const passphrase = requireSyncPassphrase(profile);
	const state = await loadSyncState(agentDir);
	...
	let merged = options.mode === "pull" && state.lastRevisionId === undefined ? base : local;
	...
	if (soleTip) {
		...
		if (summarizeConfigSnapshot(remote).payloadHash === payloadHash) {
			...
			return {
				status: state.lastRevisionId === soleTip.revisionId ? "unchanged" : "adopted",
	...
	if (options.dryRun) {
		return { status: "published", tips: graph.tips.length, quarantined: graph.quarantined.length };
	}
	await applyConfigSnapshot(agentDir, authStorage, merged, { replace: true });
...
export async function resolveConfigurationConflict(
	...
	choice: "ours" | "theirs",
	target?: string,
): Promise<{ resolved: boolean; remaining: ConfigConflictEntry[]; publicationId?: string }> {
	const profile = await loadSyncProfile(agentDir);
	...
	if (!result.merged) {
		...
		return { resolved: false, remaining: result.conflicts };
	}
