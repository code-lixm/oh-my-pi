export const SETTINGS_ZH_CN_MESSAGES: Record<string, string> = {
	...
	"System default: replace full frames with a half-cell color rail, matching translucent-looking tint, and vertical breathing room; native selection may include the rail glyph and ordinary whitespace":
		"系统默认：用半字格宽色轨、同色透明感底纹和上下留白替代完整边框；终端原生框选可能包含色轨字符及普通空白",
	Images: "图片",
	...
	"Fallback ({role}): {from} -> {to}": "回退（{role}）：{from} → {to}",
	"Fallback ({role}) succeeded on {model}": "（{role}）已在 {model} 上成功回退",
	"Primary endpoint ({role}) recovered: {from} -> {to}": "主端点（{role}）已恢复：{from} -> {to}",
	"Fallback: {url}": "备用：{url}",
	...
	"cancel job": "取消任务",
	Detail: "详情",
	"lines {start}-{end} of {total}": "第 {start}-{end} 行，共 {total} 行",
	"Recent tools": "最近工具",
	"The selected job has no focusable agent.": "选中的任务没有可聚焦的代理。",
	"Cancellation requested.": "已请求取消。",
	"The selected job is no longer running.": "选中的任务已不在运行。",
	Work: "工作内容",
	Intent: "当前意图",
	"Current tool": "当前工具",
	Owner: "所有者",
	...
	"{count} earlier jobs": "前面还有 {count} 个任务",
	"{count} later jobs": "后面还有 {count} 个任务",
	Activity: "活动",
	Progress: "进度",
	Duration: "运行时间",
	"Dynamic summary": "动态摘要",
	"Waiting for Main": "等待 Main",
	"Task result": "任务结果",
	"Report status": "上报状态",
	"Delivery pending": "等待上报",
	"Delivering to Main": "正在上报 Main",
	"Delivered to Main": "已送达 Main",
	"Main unavailable": "Main 不可用",
	previous: "上一个",
	next: "下一个",
	"Running tasks": "运行中",
	...
	All: "全部",
	"Parked: {note}": "已暂停：{note}",
	"Idle: {note}": "空闲：{note}",
	"Queued: waiting for runnable slot": "排队中：等待可运行槽位",
	...
	"Interrupt now and send the draft immediately": "立即中断并发送当前输入",
	"Fork the current session now and continue in the copy": "立即分叉当前会话并在副本中继续",
	...
	"Pause the main agent for up to 30 seconds if the advisor falls behind by this many turns. Off disables catch-up delays.":
		"当审阅助手模块落后达到指定轮数时，暂停主代理最多 30 秒等待其追赶。关闭后不再等待。",
	...
	"Restoring session… {completed}/{total}": "正在恢复会话… {completed}/{total}",
	...
	"Saved {scope} WATCHDOG.yml — {count} advisor{s} active.":
		"已保存 {scope} WATCHDOG.yml — 已启用 {count} 项审阅助手。",
	...
	Main: "主代理",
	"Enter:open transcript": "Enter:打开转录",
	"Enter:send message · Esc:cancel message": "Enter:发送消息 · Esc:取消消息",
	"Enter:switch session": "Enter:切换会话",
	"j/k:select · {enterAction} · Esc/←←:close": "j/k:选择 · {enterAction} · Esc/←←:关闭",
	"f:focus": "f:聚焦",
	"m:message": "m:发消息",
	"p:Main": "p:主代理",
	"r:revive": "r:恢复",
	...
	"Not started": "未开始",
	"Not started: pending prompt": "未开始：等待提示",
	Running: "运行中",
	"Running: {activity}": "运行中：{activity}",
	Completed: "已完成",
	...
	"Advisor found {count} blocker": "审阅助手发现 {count} 个阻断问题",
	"Advisor found {count} blockers": "审阅助手发现 {count} 个阻断问题",
	"Advisor found {count} concern": "审阅助手发现 {count} 条隐患",
	"Advisor found {count} concerns": "审阅助手发现 {count} 条隐患",
	"Advisor found {count} issue": "审阅助手发现 {count} 条问题",
	"Advisor found {count} issues": "审阅助手发现 {count} 条问题",
	"Advisor found {count} nit": "审阅助手发现 {count} 条小建议",
	"Advisor found {count} nits": "审阅助手发现 {count} 条小建议",
	...
	complete: "完成",
	"checkpoint complete": "完整",
	"checkpoint partial": "不完整",
	"checkpoint corrupt": "损坏",
	...
	"Plan ready for review.": "计划已准备好，等待审阅。",
	...
	"Install a plugin (interactive browser if no args)": "安装插件（无参数则进入交互式浏览器）",
	"Uninstall a plugin (selector if no args)": "卸载插件（无参数则进入选择器）",
	"[--force] [name@marketplace]": "[--force] [name@marketplace]",
	"Enable the advisor": "启用审阅助手",
	"Disable the advisor": "禁用审阅助手",
	"Show advisor status": "显示审阅助手状态",
	"Copy the advisor's transcript to clipboard": "复制审阅助手的转录到剪贴板",
	...
	"[prompt]": "[prompt]",
	"Re-open the plan review for the latest plan (plan mode only)": "重新打开最近计划的审阅界面（仅计划模式）",
	"Toggle vibe mode (direct persistent fast/good worker sessions; read-only toolset)":
		"切换 Vibe 模式（直接持久的 fast/good worker 会话；只读工具集）",
	"Toggle goal mode (persistent autonomous objective for this session)": "切换目标模式（此会话的持久自主目标）",
	"[objective]": "[objective]",
	"Interview and refine a goal before enabling goal mode": "访谈并完善目标，然后启用目标模式",
	"[rough objective]": "[rough objective]",
	"Toggle loop mode. While enabled, the next prompt you send re-submits after every yield. Esc cancels the current iteration; /loop again to disable.":
		"切换循环模式。启用后，你发送的下一条提示词会在每次 yield 后重新提交。Esc 取消当前迭代；再次使用 /loop 可禁用。",
	"[count|duration] [prompt]": "[count|duration] [prompt]",
	"Queue a message for after the agent yields": "排队一条消息，在代理 yield 后发送",
	"<message>": "<message>",
	"Switch model for this session": "切换此会话的模型",
	"Show current model selection": "显示当前模型选择",
	"Switch model for this session (same as alt+p)": "切换此会话的模型（同 alt+p）",
	...
	"Switch to a fast/cheap model at the next action (works even without --prewalk)":
		"下一次操作切换到快速/低价模型（即使没有 --prewalk 也生效）",
	"Prewalk at the next action": "下一次操作进行预扫描",
	"Toggle the advisor (a second model that reviews each turn and injects notes)":
		"切换审阅助手（一个在每轮后评审并注入提示的第二个模型）",
	"Toggle advisor": "切换审阅助手",
	"[on|off|status|dump [raw]|configure]": "[on|off|status|dump [raw]|configure]",
	"Export session to HTML file": "将会话导出为 HTML 文件",
	"[path]": "[path]",
	"Copy session transcript to clipboard (and write LLM request JSON to tmp)":
		"将会话记录复制到剪贴板（并将 LLM request JSON 写入 tmp）",
	"Return full transcript as plain text, with LLM request JSON path":
		"以纯文本返回完整会话记录，并附带 LLM request JSON 路径",
	"Share session via an encrypted link (share server or secret gist)":
		"通过加密链接分享会话（分享服务器或 secret gist）",
	"Share this session live via a relay": "通过 relay 实时分享此会话",
	"[start|view|stop|status] [relayUrl]": "[start|view|stop|status] [relayUrl]",
	"Join a shared collab session": "加入共享协作会话",
	"<link>": "<link>",
	"Leave the collab session": "离开协作会话",
	"Toggle browser headless vs visible mode": "切换浏览器 headless/visible 模式",
	"[headless|visible]": "[headless|visible]",
	"Pick text or code from the conversation to copy": "从对话中选择文本或代码以复制",
	"View or modify the agent's todo list": "查看或修改代理的 todo 列表",
	"Manage todos": "管理 todos",
	"<subcommand>": "<subcommand>",
	"Append a task; phase fuzzy-matched or auto-created": "追加任务；phase 模糊匹配或自动创建",
	"[<phase>] <task...>": "[<phase>] <task...>",
	"Session management commands": "会话管理命令",
	"Show session information": "显示会话信息",
	...
	"Create a new fork from a previous message": "从之前的消息创建新 fork",
	"Navigate session tree (switch branches)": "导航会话树（切换分支）",
	"Login with OAuth provider": "使用 OAuth 提供方登录",
	"[provider|redirect URL]": "[provider|redirect URL]",
	"Logout from OAuth provider": "退出 OAuth 提供方登录",
	"[provider]": "[provider]",
	"Manage MCP servers (add, list, remove, test)": "管理 MCP 服务器（添加、列出、删除、测试）",
	"Manage MCP servers": "管理 MCP 服务器",
	"Add a new MCP server": "添加新的 MCP 服务器",
	"<name> [--scope project|user] [--url <url>] [-- <command...>]":
		"<name> [--scope project|user] [--url <url>] [-- <command...>]",
	"Search Smithery registry and deploy an MCP server": "搜索 Smithery registry 并部署 MCP 服务器",
	"<keyword> [--scope project|user] [--limit <1-100>] [--semantic]":
		"<keyword> [--scope project|user] [--limit <1-100>] [--semantic]",
	"Manage SSH hosts (add, list, remove)": "管理 SSH 主机（添加、列出、删除）",
	"Manage SSH connections": "管理 SSH 连接",
	"Add an SSH host": "添加 SSH 主机",
	"<name> --host <host> [--user <user>] [--port <port>] [--key <keyPath>]":
		"<name> --host <host> [--user <user>] [--port <port>] [--key <keyPath>]",
	"Start a new session": "启动新会话",
	"Reset provider stream state without changing the local transcript": "重置提供方流状态，而不更改本地会话记录",
	"Delete the current session and start a new one": "删除当前会话并启动新会话",
	"Manually compact the session context": "手动压缩会话上下文",
	"Compact the conversation": "压缩对话",
	"Drop heavy content from context (tool results, large blocks)": "从上下文中丢弃占用量大的内容（工具结果、大块内容）",
	"Shake heavy content out of the conversation context": "清理对话上下文中占用量大的内容",
	"[elide|images]": "[elide|images]",
	"Hand off session context to a new session": "将会话上下文交接到新会话",
	"[focus instructions]": "[focus instructions]",
	"Resume a different session": "恢复另一个会话",
	"[session id]": "[session id]",
	"Ask an ephemeral side question using the current session context": "使用当前会话上下文提一个临时旁支问题",
	"<question>": "<question>",
	"Run a full background agent on tangential work": "让完整后台代理处理旁支工作",
	"<work>": "<work>",
	"Forge a TTSR rule from a complaint to stop a recurring behavior": "根据反馈生成 TTSR 规则，以阻止反复出现的行为",
	"<complaint>": "<complaint>",
	...
	"Manage memory": "管理记忆",
	"Refresh auto-refresh models bank-wide, or one model by id": "刷新全模型库的自动刷新模型，或按 ID 刷新单个模型",
	"Rename the current session": "重命名当前会话",
	"<title>": "<title>",
	"Move the current session to a different directory": "将当前会话移动到其他目录",
	"[<path>]": "[<path>]",
	"Exit the application": "退出程序",
	"Manage marketplace plugin sources and installed plugins": "管理 marketplace 插件源和已安装插件",
	"Manage plugins from marketplaces": "管理 marketplaces 中的插件",
	"View and manage installed plugins": "查看并管理已安装插件",
	"Manage plugins": "管理插件",
	"[list|enable|disable]": "[list|enable|disable]",
	"Reload all plugins (skills, commands, hooks, tools, agents, MCP)":
		"重新加载所有插件（skills、commands、hooks、tools、agents、MCP）",
	"Reload all plugins": "重新加载所有插件",
	"Force next turn to use a specific tool": "强制下一轮使用指定工具",
	"<tool-name> [prompt]": "<tool-name> [prompt]",
	"Freeze all agents (main, subagents, advisor) until resumed": "冻结所有代理（main、subagents、advisor），直到恢复",
	"Quit the application": "退出程序",

	// Dynamic slash-command autocomplete descriptions (registry runtime paths).
	"Plan: disabled in settings": "计划模式在设置中已禁用",
	"Plan: blocked by goal mode": "计划：被目标模式阻止",
	"Plan review: available": "计划审阅：可进行",
	"Plan review: plan mode inactive": "计划审阅：计划模式未启用",
	"Plan: on": "计划：已开启",
	"Plan: on ({file})": "计划：已开启（{file}）",
	"Plan: off": "计划：已关闭",
	"Vibe: on": "Vibe：已开启",
	"Vibe: off": "Vibe：已关闭",
	"Vibe: blocked by plan mode": "Vibe：被计划模式阻止",
	"Vibe: blocked by goal mode": "Vibe：被目标模式阻止",
	"Goal: disabled in settings": "目标模式在设置中已禁用",
	"Goal: off": "目标：已关闭",
	"Goal: blocked by plan mode": "目标：被计划模式阻止",
	"Goal: {status} ({detail})": "目标：{status}（{detail}）",
	"Loop: off": "循环：已关闭",
	"Loop: paused": "循环：已暂停",
	"Loop: on ({detail})": "循环：已启用（{detail}）",
	"Loop: on (repeating prompt)": "循环：已启用（重放提示）",
	"Loop: on (waiting for next prompt)": "循环：已启用（等待下一条提示）",
	"Model: {provider}/{id}": "模型：{provider}/{id}",
	"Model: none selected": "模型：未选择",
	"Fast: {status}": "快速模式：{status}",
	"Advisor: on ({n} advisors)": "审阅助手：已启用（共 {n} 项）",
	"Advisor: on ({provider}/{id})": "审阅助手：已启用（{provider}/{id}）",
	"Advisor: configured, no model": "审阅助手：已配置，但模型未指定",
	"Advisor: off": "审阅助手：已关闭",
	"Collab: hosting ({n} guests)": "协作：宿主中（{n} 位访客）",
	"Collab: guest": "协作：访客",
	"Collab: read-only guest": "协作：只读访客",
	"Collab: off": "协作：未启用",
	"Leave collab: hosting": "退出协作：宿主模式",
	"Leave collab: guest": "退出协作：访客模式",
	"Leave collab: not in collab": "退出协作：未在协作中",
	"Browser: disabled": "浏览器：已禁用",
	"Browser: headless": "浏览器：无头模式",
	"Browser: visible": "浏览器：可见模式",
	"Todos: none": "待办：无",
	"Todos: {open} open ({inProgress} in progress, {completed} done)":
		"待办：{open} 个待处理（{inProgress} 个进行中，{completed} 个已完成）",
	"Jobs: none": "后台任务：无",
	"Jobs: {running} running, {recent} recent": "后台任务：{running} 个进行中，{recent} 个近期",
	"Tools: none available": "工具：无",
	"Tools: {active} active / {all} available": "工具：{active} 个启用 / {all} 个可访问",
	"Context: unavailable": "上下文：未知",
	"Context: {percent}% ({tokens}/{contextWindow})": "上下文：{percent}%（{tokens}/{contextWindow}）",
	"Login: choose provider": "登录：选择提供方",
	"Login: waiting for {provider} callback": "登录：等待 {provider} 回调",
	"Fresh: unavailable while streaming": "Fresh：流式输出中暂不可用",
	"Fresh: ready": "Fresh：就绪",
	"Compact: context {percent}% used": "压缩：上下文已用 {percent}%",
	"Compact: context unavailable": "压缩：上下文未知",
	"Force: no active tools": "强制：未启用任何工具",
	"Force: {count} active tools": "强制：当前 {count} 个工具启用",
	"shadowed by": "被",
	version: "版本",
	scope: "作用范围",
	"install path": "安装路径",
	...
	"Configure sign-in and web search providers": "配置登录和 Web 搜索提供方",
	...
	"Share a read-only link (guests can watch, not prompt)": "分享只读链接（访客可观看但不能输入）",
	"Show link + participants": "显示链接与参与者",
	"Stop sharing": "停止分享",
	"Switch to headless mode": "切换到无界面模式",
	"Switch to visible mode": "切换到可见模式",
	"Open todos in $EDITOR (Markdown round-trip)": "在 $EDITOR 中打开待办（Markdown 往返）",
	"Copy todos as Markdown to clipboard": "复制待办为 Markdown 到剪贴板",
	"Write todos as Markdown to a file (default: TODO.md)": "将待办写入文件（默认 TODO.md）",
	...
	"Remove task/phase/all (fuzzy-matched)": "删除任务/阶段/全部（模糊匹配）",
	...
	"Remove an MCP server": "删除 MCP 服务器",
	"Test connection to a server": "测试服务器连接",
	"Reauthorize OAuth for a server": "为服务器重新授权 OAuth",
	"Remove OAuth auth from a server": "移除服务器的 OAuth 认证",
	"Enable an MCP server": "启用 MCP 服务器",
	"Disable an MCP server": "禁用 MCP 服务器",
	"Login to Smithery and cache API key": "登录 Smithery 并缓存 API key",
	"Remove cached Smithery API key": "移除已缓存的 Smithery API key",
	"Reconnect to a specific MCP server": "重新连接指定的 MCP 服务器",
	"Force reload MCP runtime tools": "强制重新加载 MCP 运行时工具",
	"List available resources from connected servers": "列出已连接服务器的可用资源",
	"List available prompts from connected servers": "列出已连接服务器的可用提示词",
	"Show notification capabilities and subscriptions": "显示通知能力与订阅",
	"Show help message": "显示帮助信息",
	"List all configured SSH hosts": "列出所有已配置的 SSH 主机",
	"Remove an SSH host": "删除 SSH 主机",
	"Strip tool results + large blocks (default)": "精简工具结果与大块内容（默认）",
	"Strip image blocks": "精简图片块",
	...
	"List mental models on the active bank": "列出当前 bank 上的心智模型",
	"Show one mental model (id required)": "显示一个心智模型（需指定 id）",
	"Diff the change history of a mental model": "对比心智模型的变更历史",
	"Create any built-in mental models that are missing": "创建缺失的内置心智模型",
	"Delete a mental model from the bank (id required)": "从 bank 删除心智模型（需指定 id）",
	"Re-pull the cached <mental_models> block": "重新拉取缓存的 <mental_models> 块",
	"Add a marketplace source": "添加 marketplace 源",
	"Remove a marketplace source": "移除 marketplace 源",
	"Update marketplace catalog(s)": "更新 marketplace 目录",
	...
	"List all installed plugins (npm + marketplace)": "列出所有已安装插件（npm + marketplace）",
	"Enable a marketplace plugin": "启用 marketplace 插件",
	"Disable a marketplace plugin": "禁用 marketplace 插件",
	"<name> [--scope project|user]": "<name> [--scope project|user]",
	"<name>": "<name>",
	"[<focus>]": "[<focus>]",
	...
	"Using tool": "正在使用工具",
	"Delegating work": "正在委派工作",
	"Retrying request": "正在重试请求",
	"Compacting context": "正在压缩上下文",
	"Waiting for user": "等待用户",
	"Waiting for peer": "等待协作者",
	"Finishing response": "正在完成回复",
	Idle: "空闲",
	Active: "活跃",
	Quiet: "暂时无活动",
	"Suspected stall": "疑似停滞",
	Blocked: "等待中",
	...
	"Default Interval": "默认间隔",
	"Delivery Mode": "投递模式",
	steer: "立即引导",
	follow_up: "排队跟进",
	"Enable Scheduling": "启用调度",
	"Enable persisted scheduled prompts for the current session": "为当前会话启用持久化定时提示",
	"Default delivery mode used by /schedule": "/schedule 使用的默认投递模式",
	Autonomous: "自主模式",
	"Enable Autonomous Mode": "启用自主模式",
	"Enable automatic continuation without waiting for user input": "无需等待用户输入即可自动继续",
	"Max Continuations": "最大继续次数",
	"Maximum automatic continuations after a turn completes": "每轮完成后的最大自动继续次数",
	"Max Turns": "最大轮数",
	"Maximum turns in one autonomous run": "单次自主运行的最大轮数",
	"Max Tokens": "最大 Token 数",
	"Maximum tokens available to one autonomous run": "单次自主运行可使用的最大 Token 数",
	"Timeout (ms)": "超时（毫秒）",
	"Maximum duration in milliseconds for one autonomous run": "单次自主运行的最长时间（毫秒）",
	"Gate Commands": "质量门命令",
	"Commands that must succeed before autonomous work can continue": "自主工作继续前必须成功的命令",
	"Gate Max Retries": "质量门最大重试次数",
	"Maximum retries for each autonomous gate command": "每条自主质量门命令的最大重试次数",
	"Gate Timeout (ms)": "质量门超时（毫秒）",
	"Maximum duration in milliseconds for each autonomous gate command": "每条自主质量门命令的最长时间（毫秒）",
	Refinement: "持续优化",
	"Enable /refine": "启用 /refine",
	"Enable /refine and continual harness updates": "启用 /refine 与持续 harness 更新",
	"Auto Refine Turns": "自动优化轮数",
	"Run refinement automatically after this many turns": "达到此轮数后自动执行持续优化",
	"Auto Refine Cooldown (ms)": "自动优化冷却（毫秒）",
	"Minimum milliseconds between automatic refinements": "两次自动优化之间的最小毫秒数",
	"Python Skills": "Python 技能",
	"Enable Python Skills": "启用 Python 技能",
	"Enable bundled Python skills": "启用内置 Python 技能",
	"Trust Mode": "信任模式",
	"Choose whether Python skills require confirmation before running": "选择 Python 技能运行前是否需要确认",
	RLM: "RLM",
	"RLM Max Depth": "RLM 最大深度",
	"Maximum RLM child recursion depth (root is depth 0); the effective cap is the stricter of this and task.maxRecursionDepth.":
		"RLM 子代理最大递归深度（根为 0）；实际限制取该值与 task.maxRecursionDepth 中更严格者。",
	"[{id}] {source} {status}": "[{id}] {source} {status}",
	"Next run: {nextRunAt}": "下次运行：{nextRunAt}",
	"Schedule not found: {id}": "未找到调度：{id}",
	"Refinement {id} was not found": "未找到持续优化记录 {id}",
	"{source} scheduling is disabled": "{source} 调度已禁用",
	"Usage: /schedule add <schedule> -- <prompt>": "用法：/schedule add <schedule> -- <prompt>",
	"Scheduling is unavailable in this session.": "当前会话无法使用调度。",
	"No scheduled prompts.": "没有定时提示。",
	"Cron Jobs — this session's lifecycle": "定时任务 — 跟随当前会话生命周期",
	"{count} active": "{count} 个活跃",
	"{count} paused": "{count} 个已暂停",
	"{count} finished": "{count} 个已结束",
	"No scheduled prompts for this session.": "当前会话暂无定时任务。",
	"Create one with /cron add, /schedule add, or the cron tool.": "使用 /cron add、/schedule add 或 cron 工具创建。",
	Job: "任务",
	Schedule: "计划",
	Status: "状态",
	"Next run": "下次运行",
	...
	"pause/resume": "暂停/恢复",
	"run now": "立即运行",
	"Only active or paused jobs can be paused or resumed.": "只有活跃或已暂停的任务可以暂停/恢复。",
	"The selected job cannot be cancelled.": "所选任务无法取消。",
	"Only active jobs can be run now.": "只有活跃的任务可以立即运行。",
	"Job prompt delivered into the session.": "任务提示已投递到会话。",
	"{count} more lines": "还有 {count} 行",
	"schedule is required for add": "add 操作需要 schedule",
	"prompt is required for add": "add 操作需要 prompt",
	"jobId is required for {op}": "{op} 操作需要 jobId",
	"Scheduled prompt created (follows this session's lifecycle): {job}":
		"已创建定时任务（跟随当前会话生命周期）：{job}",
	"Open the cron management list for this session's scheduled tasks": "打开当前会话定时任务的管理列表",
	"Scheduling is disabled. Enable schedule.enabled first.": "调度已禁用。请先启用 schedule.enabled。",
	"Scheduled prompt created: {job}": "已创建定时提示：{job}",
	"Usage: /schedule {verb} <id>": "用法：/schedule {verb} <id>",
	"Usage: /schedule update <id> <schedule> -- <prompt>": "用法：/schedule update <id> <schedule> -- <prompt>",
	"Scheduled prompt updated: {job}": "已更新定时提示：{job}",
	"Usage: /schedule <add|list|pause|resume|cancel|update> ...":
		"用法：/schedule <add|list|pause|resume|cancel|update> ...",
	"Continual harness is disabled. Enable refinement.enabled first.":
		"持续 harness 已禁用。请先启用 refinement.enabled。",
	"Usage: /refine status": "用法：/refine status",
	"{kind}={count}": "{kind}={count}",
	"{id}: {scope}: {summary}": "{id}：{scope}：{summary}",
	"Continual harness: {counts}": "持续 harness：{counts}",
	"Recent refinements:": "最近的持续优化：",
	"Usage: /refine rollback <result-id> [--local|--global]": "用法：/refine rollback <result-id> [--local|--global]",
	"Rollback completed: {id}": "回滚已完成：{id}",
	"Refinement completed and the supplemental prompt was refreshed.": "持续优化已完成，补充提示词已刷新。",
	"Last gate failure: {command} ({exitText}, attempt {attempt}/{maxRetries}).":
		"上次质量门失败：{command}（{exitText}，第 {attempt}/{maxRetries} 次）。",
	"Last gate failure: none.": "上次质量门失败：无。",
	"Autonomous mode is {status}.": "自主模式当前为{status}。",
	"Timeout: {timeoutMs}ms; gates: {gates}.": "超时：{timeoutMs} 毫秒；质量门：{gates}。",
	"Autonomous mode is not available yet. It will be enabled after integration wiring.":
		"自主模式暂不可用，将在完成集成接线后启用。",
	"Autonomous mode enabled with quality gate: {command}.": "自主模式已启用，质量门：{command}。",
	"Autonomous mode enabled.": "自主模式已启用。",
	"Autonomous mode disabled.": "自主模式已禁用。",
	"Autonomous quality gates cleared.": "自主质量门已清空。",
	"Usage: /autonomous gate add <command>": "用法：/autonomous gate add <command>",
	"Autonomous quality gate added: {command}.": "已添加自主质量门：{command}。",
	"Usage: /autonomous budget <continuations|turns|tokens|time> <positive integer>":
		"用法：/autonomous budget <continuations|turns|tokens|time> <positive integer>",
	"Autonomous {budgetType} budget set to {value}.": "自主模式的 {budgetType} 预算已设为 {value}。",
	"Usage: /autonomous [on [gate-command]|off|status|gate add <command>|gate clear|budget <continuations|turns|tokens|time> <n>]":
		"用法：/autonomous [on [gate-command]|off|status|gate add <command>|gate clear|budget <continuations|turns|tokens|time> <n>]",
	"Unable to configure autonomous mode: {error}": "无法配置自主模式：{error}",
	"Autonomous: unavailable": "自主模式：不可用",
	"Autonomous: {status}": "自主模式：{status}",
	"Install Python skill {name}?": "安装 Python 技能 {name}？",
	"OMP will install {packagePath} into its dedicated Python skill environment.":
		"OMP 将把 {packagePath} 安装到专用的 Python 技能环境中。",
	"Approved Python skills run in that dedicated skill-venv interpreter.":
		"获准的 Python 技能将在该专用 skill-venv 解释器中运行。",
	"Declared dependencies:": "声明的依赖：",
	"No declared third-party dependencies": "未声明第三方依赖",
	"Review the trajectory and update the continual harness": "审查当前轨迹并更新持续 harness",
	"Show harness entry counts and recent refinements": "显示 harness 条目数量和最近的持续优化",
	"Rollback a refinement": "回滚持续优化",
	"Manage persisted scheduled prompts for the current session": "管理当前会话的持久化定时提示",
	"Create a scheduled prompt": "创建定时提示",
	"List scheduled prompts": "列出定时提示",
	"Pause a schedule": "暂停调度",
	"Resume a schedule": "恢复调度",
	"Cancel a schedule": "取消调度",
	"Update a schedule": "更新调度",
	"Control host-owned autonomous continuation and quality gates": "控制由主机管理的自主继续与质量门",
	"Control autonomous continuation": "控制自主继续",
	"Enable autonomous continuation": "启用自主继续",
	"Disable autonomous continuation": "禁用自主继续",
	"Show autonomous budgets and gate status": "显示自主预算与质量门状态",
	"Add or clear quality gates": "添加或清空质量门",
	"Set a continuation, turn, token, or time budget": "设置继续次数、轮数、Token 或时间预算",
	cron: "定时任务",
	"Continual harness refinement is disabled or unavailable.": "持续 harness 优化已禁用或不可用。",
	"resultId is required for rollback": "回滚需要 resultId",
