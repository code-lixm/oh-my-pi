...
/**
 * Emit `session_shutdown`, dispose file fallback registrations, and clear
 * timers owned by an extension runner.
 *
 * Returns whether any shutdown handlers were present. Fallback disposal and timer
 * cleanup run even when a handler fails so no session-bound handler outlives its host.
 */
...
export class ExtensionRunner {
	...
	/** Process-wide fallback registrations bound to this runner's live session context. */
	...
	/** Session that owns this runner; used to attribute a brokered mutation. */
	...
	/** Remove all process-wide fallback registrations installed by this runner. */
