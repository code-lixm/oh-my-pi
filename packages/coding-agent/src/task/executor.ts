...
function resolveSubagentInheritedRetryFallbackChain(
	...
	const fallbackChain =
		(modelRole !== undefined ? configuredChains?.[modelRole] : undefined) ?? configuredChains?.default;
	if (
...
export interface ExecutorOptions {
	...
	/** Stable user-facing task name shown by Agent Hub; distinct from the agent profile name. */
	displayName?: string;
	task: string;
	assignment?: string;
...
export async function finalizeSubagentLifecycle(args: {
	...
	/** Process-wide cap for adopted idle subagents kept live. <= 0 disables the count cap. */
	...
	AgentLifecycleManager.global().adopt(
		args.id,
		{
			...
			maxLiveIdleAgents: args.maxLiveIdleAgents,
...
export async function runSubprocess(options: ExecutorOptions): Promise<SingleResult> {
	...
	const effectiveModelRole = modelRole ?? resolveExplicitModelRole(modelOverride ?? agent.model, settings);
	const startTime = Date.now();
	...
	// Per-agent advisor decisions take precedence over the legacy blanket
	// `advisor.subagents` setting. A false selection is an explicit opt-out;
	// only an absent selection falls through to the global default.
	...
	const advisorEnabled =
		advisorSelection === undefined ? settings.get("advisor.subagents") : advisorSelection !== false;
	const advisorModel =
		advisorSelection !== undefined && advisorSelection !== false ? advisorSelection.model : undefined;
	const subagentSettings = createSubagentSettings(
		settings,
		{
			...
			...(advisorEnabled ? { "advisor.enabled": true } : undefined),
			...(advisorModel ? { modelRoles: { ...settings.getModelRoles(), advisor: advisorModel } } : undefined),
		},
	...
	// <= 0 disables time-based parking; the independent live-idle count cap may still park it.
	const agentIdleTtlMs = Math.trunc(Number(settings.get("task.agentIdleTtlMs") ?? 420_000) || 0);
	// Process-wide count cap for live idle adopted subagents. Independent of TTL: <= 0 disables it.
	const maxLiveIdleAgents = Math.max(0, Math.trunc(Number(settings.get("task.maxLiveIdleAgents") ?? 8) || 0));
	...
	// Collaboration stays available to agents that can act on the workspace. A
	// read-only agent must not gain process supervision through hub: `hub start`
	// can execute arbitrary binaries and would bypass its declared tool boundary.
	if (toolNames && !options.restrictToolNames && !isReadOnlyAgent(agent) && !toolNames.includes("hub")) {
		...
		try {
			...
			const inheritedRetryFallbackChain =
				configuredModelPatterns.length === 1
					? resolveSubagentInheritedRetryFallbackChain(subagentSettings, modelRegistry, effectiveModelRole)
					: undefined;
			const {
				model,
			...
			session.sessionManager.appendSessionInit({
				...
				advisor: advisorEnabled ? (advisorModel ?? "on") : undefined,
				outputSchema,
				outputSchemaMode: options.outputSchemaMode,
