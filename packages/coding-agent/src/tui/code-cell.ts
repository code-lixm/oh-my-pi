...
import { type OutputBlockBorderStyle, outputBlockContentWidth, renderOutputBlock } from "./output-block";
...
export function renderMarkdownCell(options: MarkdownCellOptions, theme: Theme): string[] {
	...
	const innerWidth = Math.max(20, outputBlockContentWidth(width, undefined, options.borderStyle));
