/** Cursor exec bridge tools absent from the model-facing registry. */
...
import { FffGrepTool, type FffGrepToolOptions } from "./tools/fff-tools";
/**
...
/** Build wrapped, bridge-only FFF grep instances for Cursor frame-specific pagination and caps. */
export function createBridgeGrepFactory(
	session: ToolSession,
	extensionRunner: ExtensionRunner,
): (options?: FffGrepToolOptions) => AgentTool {
	return options => new ExtensionToolWrapper(new FffGrepTool(session, options) as Tool, extensionRunner);
}
