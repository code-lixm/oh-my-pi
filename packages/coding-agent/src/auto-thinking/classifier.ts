...
import { getPromptLocale, type PromptLocale, selectPrompt } from "../prompts/prompt-locale";
...
/**
 * Rendered classifier prompts, keyed by locale and whether `max` is offered as
 * a label. Four variants only, so each is memoized on first use.
 */
const DIFFICULTY_SYSTEM_PROMPTS: Record<PromptLocale, Partial<Record<"max" | "xhigh", string>>> = {
	en: {},
	"zh-CN": {},
};
...
function difficultySystemPromptFor(ceiling: Effort): string {
	...
	const cached = DIFFICULTY_SYSTEM_PROMPTS[locale][key];
	if (cached !== undefined) return cached;
	const rendered = prompt.render(selectPrompt(difficultySystemPrompt, difficultySystemPromptZh, locale), {
		allowMax: key === "max",
	});
	DIFFICULTY_SYSTEM_PROMPTS[locale][key] = rendered;
	return rendered;
}
