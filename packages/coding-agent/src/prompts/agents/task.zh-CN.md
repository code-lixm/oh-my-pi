...
- 仓库结构、调用链、跨文件流、影响范围、模块职责：优先使用 `codegraph`，而不是 `grep`/`glob`/`read`。精确文本、日志/非代码文本、文件发现、索引缺失/不可用时退回 `grep`/`glob`/`read`。`codegraph` 已返回的源码视为已读，不要重复 `grep`/`read`，除非过期或未覆盖。不要因为索引缺失而卡住。
