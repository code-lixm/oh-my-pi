...
- NEVER 发送纯确认、纯感谢或结束线程的消息；保持静默即可结束协作。
...
- 超时或“仍在运行”快照代表 ZERO 进展。NEVER 立即再次 `wait`；继续本地工作，确实被阻塞时取消任务或接管工作。
- 等待后台任务时省略 `timeoutMs`，让 smart backoff 决定窗口。仅为单次外部截止时间显式设置，NEVER 用于轮询循环。
- `idle`、`parked`、completed、failed、cancelled 均不是运行态。需要恢复时发送消息，NEVER 等待这些状态。
