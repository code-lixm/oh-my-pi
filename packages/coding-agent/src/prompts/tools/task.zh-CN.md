...
# 异步任务契约
- 结果会自动交付；不要轮询。
- 生成子代理后继续父任务中的独立工作。NEVER 仅因存在后台任务就调用 `hub wait`。
- `completed` 只表示代理正常结束，不代表其产物已被验收；仍需验证相关改动。
{{/if}}
...
- **协调重叠：** 同文件编辑需要显式隔离或预先分配所有权。NEVER 仅因路径重叠而缩小真正独立的批次。两个前提：
  1. 每个任务 MUST 跳过验证（build/lint/tests）——中途验证会被其他代理的编辑干扰。
  2. 预先决定跨任务契约（如 A 实现、B 使用的接口），并写入{{#if batchEnabled}}批次 `context`{{else}}任务{{/if}}，不要留给代理临时协商。
