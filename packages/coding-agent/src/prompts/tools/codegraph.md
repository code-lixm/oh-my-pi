...
<instruction>
- Structural, behavioral, flow, impact, modification, or known-source question? You MUST call this before `grep`/`find`/`read`, unless the request is solely an LSP symbol operation.
...
- Exact text, logs, configs, docs, and precise selectors → `grep`/`read`; file discovery → `find`; definition/type/references/hover/code actions → `lsp` when available.
</instruction>
<output>
...
- Ordinary fallback (runtime unavailable/error, indexing, missing/failed index, or non-Git) → immediately use `read`/`grep`/`find`/`lsp` as applicable for this project; NEVER wait, poll, or retry CodeGraph.
