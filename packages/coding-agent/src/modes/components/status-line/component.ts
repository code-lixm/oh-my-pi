...
const STATUS_USAGE_CACHE_TTL_MS = 2 * 60_000;
...
export class StatusLineComponent implements Component {
	...
	// Provider usage caching (2-min max refresh interval, normalized across provider-specific limits)
	...
	#invalidateSessionCaches(): void {
		...
		this.#cachedUsageContextKey = null;
	...
	async #runUsageRefresh(
		...
		try {
			...
			this.#applyUsageRefreshReports(session, usageContextKey, result, sequence);
		} catch {
			...
			if (signal.aborted && reportsPromise) {
				this.#observeLateUsageRefresh(session, usageContextKey, reportsPromise, sequence);
			}
		} finally {
	...
	#applyUsageRefreshReports(session: AgentSession, usageContextKey: string, reports: unknown, sequence: number): void {
		if (
			!this.#isCurrentUsageContext(session, usageContextKey) ||
	...
	#observeLateUsageRefresh(
		...
		void reportsPromise
			.then(reports => {
				this.#applyUsageRefreshReports(session, usageContextKey, reports, sequence);
			})
			.catch(() => {
				if (
					!this.#isCurrentUsageContext(session, usageContextKey) ||
	...
	#buildSegmentContext(
		...
		// Trigger background fetch (2-min max interval); render uses cached value
		this.refreshUsageInBackground();
		...
		return {
			...
			prewalk: this.session.getPrewalkStateSnapshot?.() ? { enabled: true } : null,
