...
export class ToolExecutionComponent extends Container implements NativeScrollbackLiveRegion {
	...
	/** Mark this call as executing after any approval/queue wait has completed. */
	...
	#updateDisplay(): void {
		...
		const key = `${this.#resultVersion}|${this.#expanded}|${this.#isPartial}|${this.#argsComplete ? "1" : "0"}|${this.#executionStarted ? "1" : "0"}|${this.#spinnerFrame ?? "-"}|${this.#showImages}|${getThemeEpoch()}|${getSettingsUiLocaleEpoch()}|${this.#displayInputVersion}|${this.#backgroundTaskFrozenStyled}|${TERMINAL.imageProtocol ?? "-"}|${this.#imageSizeKey()}`;
