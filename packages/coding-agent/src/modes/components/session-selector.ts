	...
	render(width: number): readonly string[] {
		...
		if (this.#filteredSessions.length === 0) {
			if (this.#showCwd) {
				...
				lines.push(
					truncateToWidth(theme.fg("muted", tSettingsUi("No sessions in current folder. Press Tab to view all.")), width),
				);
			}
			return lines;
		}
...
export class SessionSelectorComponent extends OverlayPanel {
	...
	#headerLabel(): string {
		if (this.#scopeLabel === false) return tSettingsUi(this.#title, this.#titleParams);
		const scopeLabel = this.#scopeLabel ?? (this.#scope === "all" ? "all projects" : "current folder");
		return `${tSettingsUi(this.#title, this.#titleParams)} (${tSettingsUi(scopeLabel)})`;
	}
	...
	async #toggleScope(): Promise<void> {
		...
		if (this.#scope === "folder") {
			...
			if (!global) {
				...
				this.#messageContainer.addChild(new Text(theme.fg("muted", tSettingsUi("Loading all projects…")), 0, 0));
				this.#onRequestRender?.();
				try {
	...
	#showError(message: string): void {
		...
		this.#messageContainer.addChild(
			new Text(theme.fg("error", tSettingsUi("Error: {message}", { message: replaceTabs(message) })), 0, 0),
		);
	...
	#footerLines(width: number): string[] {
		...
		return [row("", width), row(theme.fg("muted", hint), width), row("", width), bottomBorder(width)];
	}
