...
export class BtwPanelComponent extends Container {
	...
	#footerLine(): string {
		switch (this.#state) {
			...
			case "complete": {
				if (!this.isCopyable()) return theme.fg("muted", tSettingsUi("Esc dismiss"));
				const actions = [tSettingsUi("c copy")];
				if (this.#canBranch?.() ?? this.isBranchable()) actions.push(tSettingsUi("b branch to chat"));
				actions.push(tSettingsUi("Esc dismiss"));
				return theme.fg("muted", actions.join(" · "));
			}
			case "branching":
				return theme.fg("muted", `${theme.status.pending} ${tSettingsUi("Branching to chat…")}`);
			case "aborted":
