...
export class CustomEditor extends Editor {
	...
	setCollapsedText(text: string): void {
		if (hasImageMarker(text)) this.#markerManagedImages = this.pendingImages;
		this.setText(
	...
	/** Mark newly inserted image chips as deletion-managed. */
	markPendingImagesManaged(): void {
		this.#markerManagedImages = this.pendingImages;
	}

	/** Preserve legacy/restored markerless images, but compact arrays once chips or markers manage them. */
	compactPendingImageReferences(text: string): string {
		if (hasImageMarker(text)) this.#markerManagedImages = this.pendingImages;
		if (this.#markerManagedImages !== this.pendingImages) return text;
		const compacted = compactImageMarkers(text, this.pendingImages.length);
		if (!compacted) return text;
		this.pendingImages = compacted.keep.map(index => this.pendingImages[index]);
		this.pendingImageLinks = compacted.keep.map(index => this.pendingImageLinks[index]);
