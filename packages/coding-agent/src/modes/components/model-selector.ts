...
import { formatRoleDisplayLabel } from "./model-browser";
...
function makeAutoSelectedBadge(label: string, color: ThemeColor): string {
	return `${theme.fg("dim", "[")}${theme.fg(color, label)}${theme.fg("dim", ` ${tSettingsUi("auto")}]`)}`;
}
...
export class ModelSelectorComponent extends Container {
	...
	#temporaryHintText: Text | null = null;
	...
	constructor(
		...
		const hintText =
			scopedModels.length > 0
				? tSettingsUi("Showing models from --models scope")
				: tSettingsUi("Only showing models with configured API keys (see README for details)");
		this.addChild(new Text(theme.fg("warning", hintText), 0, 0));
		this.addChild(new Spacer(1));
		if (this.#temporaryOnly) {
			this.#temporaryHintText = new Text("", 0, 0);
			this.addChild(this.#temporaryHintText);
			this.addChild(new Spacer(1));
	...
	#buildMenuRoleActions(): void {
		const roleActions = getKnownRoleIds(this.#settings).map(role => {
			...
			const displayLabel = formatRoleDisplayLabel(role, this.#settings);
			const roleLabel =
				roleInfo.tag && displayLabel === roleInfo.tag ? `${roleInfo.tag} (${roleInfo.name})` : displayLabel;
			return {
				label: tSettingsUi("Set as {roleLabel}", { roleLabel }),
				role,
		...
		this.#menuRoleActions = [
			...roleActions,
			{ label: tSettingsUi("Set as DEFAULT retry fallback"), role: "default", action: "retryFallback" },
		];
	}
	...
	#getActiveProviderRefreshStatusText(): string | undefined {
		...
		return theme.fg(
			"warning",
			`  ${spinner} ${tSettingsUi("Refreshing {provider} in background...", { provider: formatProviderTabLabel(providerId) })}`,
		);
	}
	...
	#updateTabBar(): void {
		...
		const tabBar = new TabBar(tSettingsUi("Models"), tabs, getTabBarTheme(), this.#activeTabIndex);
	...
	#formatCurrentContextLimitSuffix(model: Model): string {
		return ` ${theme.status.disabled} ${tSettingsUi("context>{limit}", { limit: formatNumber(model.contextWindow ?? 0).toLowerCase() })}`;
	}
	...
	#formatDiscoveryAge(fetchedAt: number | undefined): string | undefined {
		...
		if (ageMs < 60_000) {
			return tSettingsUi("less than a minute ago");
		}
		const ageMinutes = Math.round(ageMs / 60_000);
		return tSettingsUi("{count}m ago", { count: ageMinutes });
	}
	...
	#formatDiscoveryErrorHint(error: string | undefined): string | undefined {
		...
		return `  ${tSettingsUi("Discovery failed: {error}", { error })}`;
	}
	...
	#updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			for (const role of MODEL_ROLE_IDS) {
				...
				if (!roleInfo.tag || !assigned || !modelsAreEqual(assigned.model, item.model)) continue;

				roleBadgeTokens.push(
					makeRoleBadgeToken(formatRoleDisplayLabel(role, this.#settings), roleInfo.color ?? "success", assigned),
				);
			}
			// Custom role badges
			for (const [role, assigned] of Object.entries(this.#roles)) {
				...
				const roleInfo = getRoleInfo(role, this.#settings);
				const badgeLabel = formatRoleDisplayLabel(role, this.#settings);
		...
		if (this.#errorMessage) {
			...
			const message =
				...
				(searching && activeProviderId
					? `  ${tSettingsUi("No matching models in {provider}. Switch to ALL to search every provider.", { provider: formatProviderTabLabel(activeProviderId) })}`
			...
			this.#listContainer.addChild(
				new Text(
					theme.fg("muted", `  ${tSettingsUi("Model Name: {name}", { name: selected.model.name })}`) +
						limitWarning,
					0,
					0,
				),
	...
	#updateMenu(): void {
		...
		const selectedRoleName = this.#menuSelectedRole
			? formatRoleDisplayLabel(this.#menuSelectedRole, this.#settings)
			: "";
		const headerText =
	...
	override render(width: number): readonly string[] {
		if (this.#temporaryHintText) {
			this.#temporaryHintText.setText(theme.fg("muted", tSettingsUi(TEMPORARY_MODEL_PICKER_HINT)));
		}
		const lines: string[] = [];
