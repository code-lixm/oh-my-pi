...
const CUSTOM_REQUEST_OPTION = tSettingsUi("Describe what to fix…");
...
export class CleanseCommandController {
	...
	handleEscape(): boolean {
		...
		if (!run.settled && !run.abortController.signal.aborted) {
			run.abortController.abort(new Error(tSettingsUi("Cleanse interrupted")));
			return true;
		}
		this.#close(run);
		return true;
	...
	async start(args: string): Promise<void> {
		if (this.#active) {
			this.ctx.showStatus(tSettingsUi("A /cleanse run is already active — Esc cancels it."));
			return;
		}
	...
	async #pickTarget(checkers: readonly CleanseCheckerDescriptor[]): Promise<CleanseTargetChoice> {
		const allOption = tSettingsUi("Run all {count} discovered checker{s}", {
			count: checkers.length,
			s: checkers.length === 1 ? "" : "s",
		});
		const labels = checkers.map(checker =>
			tSettingsUi("{label} — {command}", { label: checker.label, command: checker.command }),
		);
		const choice = await this.ctx.showHookSelector(tSettingsUi("Select what to cleanse"), [
			allOption,
			...labels,
	...
	async #promptRequest(): Promise<string | null> {
		const answer = await this.ctx.showHookInput(
			tSettingsUi("Describe what to detect and fix"),
			tSettingsUi('e.g. "ts errors"'),
		);
...
function parseCleanseArgs(args: string): ParsedCleanseArgs {
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		if (token === "--all" || token === "-a") {
			...
			if (!Number.isInteger(value) || value <= 0) return { ...parsed, error: cleanseUsage() };
			parsed.maxAgents = value;
		} else if (token === "--model" || token === "-m") {
			const value = tokens[++i];
			if (!value) return { ...parsed, error: cleanseUsage() };
			parsed.model = value;
		} else if (token.startsWith("-") && token.length > 1 && !/^-\d/.test(token)) {
			return { ...parsed, error: cleanseUsage() };
		} else {
...
function cleanseUsage(): string {
	return tSettingsUi("Usage: /cleanse [request] [--all] [--tests] [-n <agents>] [-m <model>]");
}
