...
function isServerResolvedCursorTodo(content: CursorExecResolvedCarrier & { arguments: unknown }): boolean {
...
export class EventController {
	...
	#serverResolvedTodoCallIds = new Set<string>();
	 ...
	 * itself is id-agnostic (routing is via `pendingTools`), so only id-keyed
	 * maps and shared-group entries need re-keying.
	 */
	#migrateStreamedToolCallId(oldId: string, newId: string): void {
		...
		if (this.#serverResolvedTodoCallIds.delete(oldId)) {
			this.#serverResolvedTodoCallIds.add(newId);
		}
		// Shared read and hub groups index their member rows by call id; rename
		// the entry so the next cumulative update does not duplicate it.
		if (pending instanceof ReadToolGroupComponent) pending.renameEntry(oldId, newId);
		if (pending instanceof HubActivityGroupComponent) pending.renameEntry(oldId, newId);
		...
		if (this.ctx.streamingComponent && event.message.role === "assistant") {
			...
			for (let contentIndex = 0; contentIndex < this.ctx.streamingMessage.content.length; contentIndex++) {
				...
				if (content.name === "todo") {
					...
					this.#resetReadGroup();
					if (!isServerResolvedCursorTodo(content)) {
						this.#suppressLiveTodoComponent(content.id);
						continue;
					}
					this.#serverResolvedTodoCallIds.add(content.id);
				}
				...
				if (
					...
					isHubPeerCommunicationToolCall(content.name, renderArgs)
				) {
					this.#toolArgsReveal.finish(content.id);
					continue;
				}
	...
	async #handleToolExecutionEnd(event: Extract<AgentSessionEvent, { type: "tool_execution_end" }>): Promise<void> {
		...
		if (!hiddenPeerCommunicationDispatch && event.toolName === "read") {
		...
		} else if (!hiddenPeerCommunicationDispatch) {
			const component = this.ctx.pendingTools.get(event.toolCallId);
			if (component) {
		...
		if (event.toolName === "todo" && !event.isError) {
			...
			this.ctx.showWarning(
				detail
					? tSettingsUi("Todo update failed: {message}", { message: detail })
	...
	async #handleAutoCompactionStart(
		...
		const actionLabel =
			event.action === "remote"
				? tSettingsUi("Auto server compaction")
	...
	async #handleAutoCompactionEnd(event: Extract<AgentSessionEvent, { type: "auto_compaction_end" }>): Promise<void> {
		...
		if (event.aborted) {
			...
			this.ctx.showWarning(tSettingsUi("Auto server compaction failed; continuing without maintenance"));
		} else {
