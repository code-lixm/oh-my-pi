...
export class ExtensionUiController {
	...
	showToolError(toolName: string, error: string): void {
		const errorText = new Text(tSettingsUi('Tool "{toolName}" error: {error}', { toolName, error }), 1, 0).setStyleFn(
			t => theme.fg("error", t),
		);
			...
			const inputGuard =
				draftEditor.getText().length > 0
					? {
							...
							hint: tSettingsUi("Finish or clear the current prompt to answer"),
	...
	async showHookConfirm(title: string, message: string, dialogOptions?: ExtensionUIDialogOptions): Promise<boolean> {
		...
		const result = await this.showHookSelector(`${title}\n${message}`, [yes, tSettingsUi("No")], dialogOptions);
		return result === yes;
	}
	...
	showExtensionError(extensionPath: string, error: string): void {
		const errorText = new Text(
			tSettingsUi('Extension "{extensionPath}" error: {error}', { extensionPath, error }),
			1,
			0,
