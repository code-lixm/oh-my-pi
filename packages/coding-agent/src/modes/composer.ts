...
export interface ComposerStartOptions extends TUIStartOptions {
...
export class Composer implements TerminalFrameProvider {
	...
	start(options: ComposerStartOptions = {}): void {
		if (this.#started || this.#stopped) return;
		this.#started = true;
		const { playWelcomeIntro, ...startOptions } = options;
		this.ui.start(startOptions);
		if (playWelcomeIntro !== false) this.playWelcomeIntro();
	}
