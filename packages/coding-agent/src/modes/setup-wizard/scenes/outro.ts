...
export function renderSetupOutro(width: number, height: number, elapsedMs: number): string[] {
	...
	const title = theme.bold(theme.fg("success", `${theme.status.success} ${tSettingsUi("Setup saved")}`));
	const subtitle = theme.fg("muted", tSettingsUi("Handing off to the normal CLI…"));
