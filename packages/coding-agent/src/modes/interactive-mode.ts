...
export class InteractiveMode implements InteractiveModeContext {
	...
	#syncTodoAutoClearTimer(): void {
		...
		const hasOpenTodos = this.todoPhases.some(phase => phase.tasks.some(task => !isClosedTodo(task)));
		if (
			!Number.isFinite(delaySeconds) ||
			delaySeconds < 0 ||
			!this.todoPhases.some(phase => phase.tasks.some(isClosedTodo)) ||
			hasOpenTodos
		)
			return;
		if (delaySeconds === 0) {
	...
	#renderTodoList(
		...
		if (this.focusedAgentId) {
			...
			const done = tasks.filter(isClosedTodo).length;
	...
	async handleGuidedGoalCommand(
		...
		try {
			...
			const kickoff = prompt.render(selectPrompt(guidedGoalInterviewPrompt, guidedGoalInterviewPromptZh), {
	...
	async handlePlanApproval(details: PlanApprovalDetails): Promise<void> {
		...
		// "Plan ready for review." and immediately re-dispatches approval in a loop.
			...
			cycle && cycle.models.length > 1
				? {
						...
						segments: cycle.models.map(entry => ({
							label: formatRoleDisplayLabel(entry.role, this.session.settings).toLowerCase(),
