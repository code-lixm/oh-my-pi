...
import securityReviewerPromptZh from "../prompts/agents/security-reviewer.zh-CN.md" with { type: "text" };
import { selectPrompt } from "../prompts/prompt-locale";
import securityCoordinatorPrompt from "../prompts/security/scan-coordinator.md" with { type: "text" };
import securityCoordinatorPromptZh from "../prompts/security/scan-coordinator.zh-CN.md" with { type: "text" };
import securityRequestPrompt from "../prompts/security/scan-request.md" with { type: "text" };
import securityRequestPromptZh from "../prompts/security/scan-request.zh-CN.md" with { type: "text" };
import securityPublishDescription from "../prompts/tools/security-publish.md" with { type: "text" };
import securityPublishDescriptionZh from "../prompts/tools/security-publish.zh-CN.md" with { type: "text" };
...
const SECURITY_WORKFLOW_FINGERPRINT = createSecurityWorkflowFingerprint([
	...
	securityRequestPromptZh,
	securityReviewerPrompt,
	securityReviewerPromptZh,
	securityPublishDescription,
	securityPublishDescriptionZh,
]);
...
async function createDefaultSecuritySession(input: SecurityScanSessionFactoryInput): Promise<AgentSession> {
	...
	const { session } = await createAgentSession({
		...
		appendSystemPrompt: selectPrompt(securityCoordinatorPrompt, securityCoordinatorPromptZh).trim(),
...
function requestText(plan: SecurityScanPlan, executionRoot: string, diffText?: string): string {
	return prompt
		.render(selectPrompt(securityRequestPrompt, securityRequestPromptZh), {
