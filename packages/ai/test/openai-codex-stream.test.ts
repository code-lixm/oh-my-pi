...
describe("openai-codex streaming", () => {
	...
	async function runCodexSseEvents(events: unknown[]) {
		...
		return { result, textEndContents, streamEvents, eventTypes };
	}
	...
	for (const testCase of [
		{
			name: "raw reasoning",
			deltaType: "response.reasoning_text.delta",
			chunks: ["raw ", "reasoning"],
		},
		{
			name: "summary reasoning",
			deltaType: "response.reasoning_summary_text.delta",
			chunks: ["summary ", "reasoning"],
		},
	] as const) {
		it(`emits ${testCase.name} deltas before response.done without replay`, async () => {
			const itemId = `rs_${testCase.name.replace(" ", "_")}`;
			const expectedThinking = testCase.chunks.join("");
			const events = [
				{
					type: "response.output_item.added",
					output_index: 0,
				...
				...(testCase.deltaType === "response.reasoning_summary_text.delta"
					? [
							{
								type: "response.reasoning_summary_part.added",
								output_index: 0,
					...
					: []),
				...testCase.chunks.map(delta => ({
					type: testCase.deltaType,
					output_index: 0,
					item_id: itemId,
					...(testCase.deltaType === "response.reasoning_summary_text.delta" ? { summary_index: 0 } : {}),
					delta,
				})),
				{
					type: "response.output_item.done",
			...
			const { result, streamEvents } = await runCodexSseEvents(events);
			const thinkingDeltas = streamEvents.filter(
				(event): event is Extract<AssistantMessageEvent, { type: "thinking_delta" }> =>
					event.type === "thinking_delta",
			);
			const terminalIndex = streamEvents.findIndex(event => event.type === "done" || event.type === "error");

			// The wire terminal event must not replay or duplicate already-streamed thinking.
			...
			expect(terminalIndex).toBe(5);
			expect(thinkingDeltas.map(event => event.delta)).toEqual([...testCase.chunks]);
			expect(thinkingDeltas.every((_, index) => streamEvents.indexOf(thinkingDeltas[index]!) < terminalIndex)).toBe(
				true,
			);

			const thinkingBlocks = result.content.filter(block => block.type === "thinking");
			expect(thinkingBlocks).toHaveLength(1);
			expect(thinkingBlocks[0]?.thinking).toBe(expectedThinking);
		});
	}
