...
export class AuthStorage {
	...
	#resolveUsageProvider(provider: Provider): UsageProvider | undefined {
		const runtimeOverride = this.#runtimeUsageProviderOverrides.get(provider)?.provider;
	...
	async #collectUsageRequests(options?: { baseUrlResolver?: (provider: Provider) => string | undefined }): Promise<{
		...
		for (const providerId of providers) {
			...
			if (entries.length === 0) {
				...
				const extensionUsageKey = extensionUsageKeyConfig
					? await this.#configValueResolver(extensionUsageKeyConfig)
					: undefined;
				const envKey = getEnvApiKey(providerId);
				const apiKey = runtimeKey ?? extensionUsageKey ?? envKey ?? this.#fallbackResolver?.(providerId);
				if (!apiKey) continue;
	...
	async fetchUsageReports(options?: {
		...
		if (
			...
			this.#runtimeUsageProviders.size === 0 &&
			this.#runtimeUsageProviderOverrides.size === 0
		) {
			return storeReports ?? null;
		}

		const collected = await this.#collectUsageRequests(options);
