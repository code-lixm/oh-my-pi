...
import { ollamaCloudRankingStrategy, ollamaCloudUsageProvider, ollamaUsageProvider } from "./usage/ollama";
...
const DEFAULT_RANKING_STRATEGIES = new Map<Provider, CredentialRankingStrategy>([
	...
	["ollama-cloud", ollamaCloudRankingStrategy],
]);
...
export class AuthStorage {
	...
	async #collectUsageRequests(options?: { baseUrlResolver?: (provider: Provider) => string | undefined }): Promise<{
		...
		if (!resolver) return { requests: [], clientOwnedProviders: new Set<string>() };
