...
export interface StrictToolSchemaViolationOptions {
	/** xAI rejects object-root anyOf/oneOf schemas after provider normalization. */
	rejectXaiRootObjectUnion?: boolean;
}

function hasObjectRootUnion(node: JsonRecord): "anyOf" | "oneOf" | undefined {
...
export function findStrictToolSchemaViolation(
	schema: unknown,
	path = "#",
	options: StrictToolSchemaViolationOptions = {},
): string | null {
	...
	const rootUnion = options.rejectXaiRootObjectUnion && path === "#" ? hasObjectRootUnion(node) : undefined;
