 ...
 * heuristics: Gemini, DeepSeek, Grok, or MiniMax M3. Exact suffix-cycle
 * detection applies to every enabled model independently of this predicate.
 *
 * Model identity is derived only from its id; provider and compatibility metadata
 * do not opt opaque aliases into semantic detection. Gemini, DeepSeek, and Grok
 * use their family token; MiniMax is limited to the catalog's exact M3 family predicate.
 */
