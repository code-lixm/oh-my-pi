...
class CodexStreamRuntime {
	...
	/** Summary deltas buffered while waiting for atomic `.done` events. */
	pendingSummaryDeltas = new Map<CodexOpenItem, string[]>();
	/** Completed parts that need a separator only if another part emits text. */
	completedSummaryParts = new Set<CodexOpenItem>();
	/** Entries whose atomic summary done events superseded legacy deltas. */
	atomicSummaryDone = new Set<CodexOpenItem>();
	...
	resetAccumulators(): void {
		...
		this.completedSummaryParts.clear();
		this.atomicSummaryDone.clear();
	}
	...
	queueSummaryDelta(entry: CodexOpenItem | null | undefined, delta: string): void {
		...
		if (this.completedSummaryParts.delete(entry)) pending.push("\n\n");
		pending.push(delta);
		this.pendingSummaryDeltas.set(entry, pending);
	}

	markSummaryPartDone(entry: CodexOpenItem | null | undefined): void {
		if (entry?.block?.type === "thinking") this.completedSummaryParts.add(entry);
	}
	markAtomicSummaryDone(entry: CodexOpenItem | null | undefined): void {
		if (entry?.block?.type === "thinking") this.atomicSummaryDone.add(entry);
	}

	hasAtomicSummaryDone(entry: CodexOpenItem | null | undefined): boolean {
		return entry ? this.atomicSummaryDone.has(entry) : false;
	}

	peekSummaryDeltas(entry: CodexOpenItem | null | undefined): string[] {
		return entry ? (this.pendingSummaryDeltas.get(entry) ?? []) : [];
	}
	...
	takeSummaryDeltas(entry: CodexOpenItem | null | undefined): string[] {
		...
		this.completedSummaryParts.delete(entry);
		return pending;
	}

	closeOpenItem(entry: CodexOpenItem | null | undefined): void {
		if (!entry) return;
		this.atomicSummaryDone.delete(entry);
	...
	#handleStreamEvent(rawEvent: Record<string, unknown>, firstTokenTime: number | undefined): number | undefined {
		...
		if (eventType === "response.reasoning_summary_text.delta") {
			...
			if (this.#sequentialCutoffSummaries) {
				// Atomic done snapshots supersede late legacy deltas for this item.
				if (!this.runtime.hasAtomicSummaryDone(entry)) this.runtime.queueSummaryDelta(entry, delta);
				return firstTokenTime;
			}
		...
		if (eventType === "response.reasoning_summary_text.done") {
			...
			if (entry?.item.type === "reasoning" && entry.block?.type === "thinking") {
				...
				this.runtime.markAtomicSummaryDone(entry);
				if (!firstTokenTime) firstTokenTime = performance.now();
				const summaryIndex =
		...
		if (eventType === "response.reasoning_summary_part.done") {
			const entry = this.runtime.openItemForEvent(rawEvent);
			const part = rawEvent.part as { text?: unknown } | undefined;
			const partText = typeof part?.text === "string" ? part.text : "";
			if (this.#sequentialCutoffSummaries) {
				if (this.runtime.hasAtomicSummaryDone(entry)) return firstTokenTime;
				if (entry && partText) {
					const pending = this.runtime.peekSummaryDeltas(entry).join("");
					const streamedPart = pending.startsWith("\n\n") ? pending.slice(2) : pending;
					const suffix = partText.startsWith(streamedPart) ? partText.slice(streamedPart.length) : partText;
					this.runtime.queueSummaryDelta(entry, suffix);
				}
				this.runtime.markSummaryPartDone(entry);
			} else if (entry?.item.type === "reasoning" && entry.block?.type === "thinking") {
				appendReasoningSummaryPartDoneText(entry.item, entry.block, partText, stream, output, entry.contentIndex);
			}
			return firstTokenTime;
		}
