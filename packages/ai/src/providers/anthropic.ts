...
export function buildAnthropicClientOptions(args: AnthropicClientOptionsArgs): AnthropicClientOptionsResult {
	...
	// already sits in `defaultHeaders` for a non-official, non-OAuth effective
	// endpoint — either our auto-built `Bearer <apiKey>` or a caller-supplied
	// custom auth scheme via `model.headers` / `ANTHROPIC_CUSTOM_HEADERS` (#3391).
	// Adding a bonus `X-Api-Key` would force the proxy to deal with two competing
	// credentials when the user explicitly asked for one.
