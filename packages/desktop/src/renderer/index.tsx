...
import {
  ...
  useCommand,
  useLanguage,
} from "@oh-my-pi/pi-web"
import type { UpdaterState } from "@oh-my-pi/pi-web/updater"
import * as Sentry from "@sentry/solid"
import type { AsyncStorage } from "@solid-primitives/storage"
import { createMemoryHistory, MemoryRouter, type BaseRouterProps } from "@solidjs/router"
import { type Accessor, createEffect, createMemo, createResource, createSignal, onCleanup, Show } from "solid-js"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.VITE_OMP_CHANNEL === "prod" &&
...
const deepLinkEvent = "omp:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__OMP__ ??= {}
  const pending = window.__OMP__.deepLinks ?? []
  window.__OMP__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `omp.desktop.window.${windowID}.last-active-url`
}
...
const createPlatform = (windowState: DesktopWindowState, runtimeVersion: Accessor<string | undefined>): Platform => {
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const platform = createPlatform(props.windowState, runtimeVersion)
  const loadLocale = async () => {
    const raw = await platform.storage?.("omp.global.dat").getItem("language")
    if (!raw) return
  ...
  createEffect(() => setRuntimeVersion(initializationData(sidecar)?.version))
  ...
  function App() {
    ...
    const effectiveDefaultServer = createMemo(() => {
      ...
      return ServerConnection.Key.make(key ?? "sidecar")
    })
    return (
