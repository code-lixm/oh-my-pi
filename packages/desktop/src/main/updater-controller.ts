...
export type { UpdaterState } from "@oh-my-pi/pi-web/updater"
...
type EnabledUpdaterControllerInput = {
  enabled: true
...
type DisabledUpdaterControllerInput = {
  enabled: false
}

type UpdaterControllerInput = EnabledUpdaterControllerInput | DisabledUpdaterControllerInput

export type UpdaterController = {
  getState: () => UpdaterState
  subscribe: (listener: (state: UpdaterState) => void) => () => boolean
  start: () => Promise<UpdaterState>
  check: () => Promise<UpdaterState>
  install: () => Promise<void>
}

export function createUpdaterController(input: UpdaterControllerInput): UpdaterController {
  if (!input.enabled) return createDisabledUpdaterController()
...
function createDisabledUpdaterController(): UpdaterController {
  const state: UpdaterState = { status: "disabled" }
  const listeners = new Set<(state: UpdaterState) => void>()
