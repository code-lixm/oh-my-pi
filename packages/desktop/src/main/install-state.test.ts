...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier OMP launch", () => {
    expect(hasExistingAppState([file("omp.settings")])).toBe(true)
    expect(hasExistingAppState([file("omp.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("omp")])).toBe(true)
  })
})
