...
test("returns an empty catalog when a ready directory runtime payload omits providers", () => {
  expect(
    selectProviderCatalog({
      explicit: true,
      directory: "/repo",
      catalog: {
        ready: true,
        providers: undefined as unknown as NormalizedProviderListResponse,
      },
    }),
...
test("returns an empty catalog when a route runtime payload omits global providers", () => {
  const global = undefined as unknown as NormalizedProviderListResponse

  expect(selectProviderCatalog({ explicit: false, global })).toEqual({ all: new Map(), connected: [], default: {} })
})
