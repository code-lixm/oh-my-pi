...
import { DialogOmpRuntime } from "./dialog-omp-runtime"
...
export function SessionHeader() {
  ...
  const sessionID = createMemo(() => {
    ...
    const value = /\/session\/([^/]+)/.exec(pathname)?.[1]
    return value ? decodeURIComponent(value) : undefined
  })
  const openOmpRuntime = () => {
    const id = sessionID()
    if (!id) return
    void dialog.show(() => <DialogOmpRuntime sessionID={id} directory={() => projectDirectory() || undefined} />)
  }
  ...
  const v2ActionsState = createMemo<SessionHeaderV2ActionsState>(() => ({
    ...
    runtimeVisible: typeof window !== "undefined" && window.location.pathname.includes("/session/"),
    runtimeLabel: language.t("session.ompRuntime.trigger"),
    onRuntimeOpen: openOmpRuntime,
              ...
              fallback={
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    <Show when={typeof window !== "undefined" && window.location.pathname.includes("/session/")}>
                      <Tooltip placement="bottom" value={language.t("session.ompRuntime.trigger")}>
                        <IconButton
                          icon="terminal"
                          variant="ghost"
                          class="titlebar-icon w-8 h-6 p-0 box-border shrink-0"
                          onClick={openOmpRuntime}
                          aria-label={language.t("session.ompRuntime.trigger")}
                        />
                      </Tooltip>
                    </Show>
                    <TooltipKeybind
...
function SessionHeaderV2Actions(props: { state: SessionHeaderV2ActionsState }) {
  ...
  return (
    <div class="flex items-center gap-2">
      ...
      <Show when={props.state.runtimeVisible}>
        <TooltipV2 class="shrink-0" placement="bottom" value={props.state.runtimeLabel}>
          <IconButtonV2
            type="button"
            variant="ghost-muted"
            size="large"
            class="!w-9 shrink-0"
            onClick={props.state.onRuntimeOpen}
            aria-label={props.state.runtimeLabel}
