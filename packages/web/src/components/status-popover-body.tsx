...
export function StatusPopoverBody(props: { shown: Accessor<boolean> }) {
  ...
  const pluginEmpty = createMemo(() => pluginEmptyMessage(language.t("dialog.plugins.empty"), "config.yml"))
              ...
              >
                <For each={mcpNames()}>
                  {(name) => {
                    ...
                    return (
                      <Show
                        when={capabilities()?.mcpWrite === true}
                        fallback={
                          <div class="flex items-center gap-2 w-full min-h-8 pl-3 pr-2 py-1 text-left">
                            <div
                              classList={{
