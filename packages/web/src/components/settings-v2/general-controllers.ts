...
export function createShellSettingsController() {
  ...
  const [shells] = createResource(
    ...
    () => serverSdk().api.runtime.shells(),
    { initialValue: [] as ShellOption[] },
  );
  const current = createMemo(() => serverSync().data.config.shell ?? "");

  return {
    available: createMemo(
      () => platform.platform === "desktop" && shells.latest.length > 0,
    ),
    current,
...
export function createAppearanceSettingsController() {
  ...
  return {
    theme: {
      ...
      current: createMemo(() =>
        themeOptions().find((option) => option.id === theme.themeId()),
      ),
      select: (id: string) => theme.setTheme(id),
    },
    scheme: {
...
export function createSoundSettingsController() {
  ...
  const channel = (
    ...
    current: createMemo(() =>
      enabled()
        ? (soundOptions.find((option) => option.id === current()) ?? noneSound)
        : noneSound,
    ),
...
export type PermissionScopeController = ReturnType<
  typeof createPermissionScopeController
>;
export type ShellSettingsController = ReturnType<
  typeof createShellSettingsController
>;
export type AppearanceSettingsController = ReturnType<
  typeof createAppearanceSettingsController
>;
export type SoundSettingsController = ReturnType<
  typeof createSoundSettingsController
>;
