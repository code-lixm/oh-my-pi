...
const PermissionScopeSetting: Component<{
    ...
    <Show when={props.controller.available()}>
      ...
      >
        <SelectV2
          ...
          current={
            options().find(
              (option) => option.value === props.controller.current(),
            ) ?? options()[0]
          }
          ...
          label={(option) => {
            if (option.id === "auto")
              return language.t("settings.general.row.shell.autoDefault");
      ...
      <SettingsListV2>
        ...
        >
          <SelectV2
            ...
            current={schemeOptions.find(
              (option) => option === props.controller.scheme.current(),
            )}
            placement="bottom-end"
            gutter={6}
            label={(option) => {
            ...
            onSelect={(option) =>
              option && props.controller.scheme.select(option)
            }
          />
        </SettingsRowV2>
        ...
        >
          <SelectV2
            ...
            onSelect={(option) =>
              option && props.controller.theme.select(option.id)
            }
          />
        </SettingsRowV2>
...
const FontSetting: Component<{
  ...
  return (
    <SettingsRowV2
      ...
      description={language.t(config().description)}
    >
      <div class="w-full sm:w-[220px]">
        <TextInputV2
          ...
          onInput={(event) =>
            props.fonts[config().input](event.currentTarget.value)
          }
...
const SoundsSection: Component<{ controller: SoundSettingsController }> = (
  props,
) => {
  const language = useLanguage();
  return (
...
export const SettingsGeneralV2: Component<{
  ...
  const [pinchZoom, { mutate: setPinchZoom }] = createResource(
    ...
    () =>
      Promise.resolve(platform.getPinchZoomEnabled?.() ?? false).catch(
        () => false,
      ),
    { initialValue: false },
  );
  ...
  const GeneralSection = () => (
    <div class="settings-v2-section">
      <SettingsListV2>
        ...
        <Show when={mobile() && import.meta.env.VITE_OMP_CHANNEL !== "prod"}>
          <SettingsRowV2
            title={language.t(
          ...
          >
            <div data-action="settings-mobile-titlebar-bottom">
              <Switch
                ...
                onChange={(checked) =>
                  settings.general.setMobileTitlebarPosition(
                    checked ? "bottom" : "top",
                  )
                }
              />
  ...
  const UpdatesSection = () => (
    <div class="settings-v2-section">
      ...
      <SettingsListV2>
        ...
        <Show when={updater.action().available}>
          <SettingsRowV2
