...
import type { ReferenceInfo } from "@/omp/types";
...
export type PromptInputV2ComposerController = PromptInputV2Interaction & {
  ...
  readonly omp: PromptInputProps["controls"]["omp"];
};
      ...
      modelControl={
        <div class="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <PromptInputV2ModelControl
            ...
            modelName={
              props.controller.model.selection.current()?.name ??
          ...
          />
          <PromptInputV2OmpControls controls={props.controller.omp} />
        </div>
      }
    />
  );
...
function PromptInputV2OmpControls(props: {
  controls: PromptInputProps["controls"]["omp"];
}) {
  const language = useLanguage();
  const compactLabel = (label: string) =>
    label.replace(/^.*?[:：៖፦]\s*/u, "");
  const thinkingLabel = (value: OmpThinkingLevel) => {
    const labels: Record<OmpThinkingLevel, string> = {
      off: language.t("prompt.omp.thinking.off"),
      auto: language.t("prompt.omp.thinking.auto"),
      minimal: language.t("prompt.omp.thinking.minimal"),
      low: language.t("prompt.omp.thinking.low"),
      medium: language.t("prompt.omp.thinking.medium"),
      high: language.t("prompt.omp.thinking.high"),
      xhigh: language.t("prompt.omp.thinking.xhigh"),
      max: language.t("prompt.omp.thinking.max"),
    };
    return labels[value];
  };
  const approvalLabel = (value: OmpApprovalMode) => {
    const labels: Record<OmpApprovalMode, string> = {
      "always-ask": language.t("prompt.omp.approval.alwaysAsk"),
      write: language.t("prompt.omp.approval.write"),
      yolo: language.t("prompt.omp.approval.yolo"),
    };
    return labels[value];
  };
  return (
    <Show when={props.controls.visible}>
      <Show when={props.controls.thinking.options.length > 1}>
        <div
          data-action="prompt-thinking"
          classList={{
            "pointer-events-none opacity-50": props.controls.thinking.loading,
          }}
        >
          <PromptInputV2Select
            title={language.t("prompt.omp.thinking.title")}
            options={props.controls.thinking.options.map((value) => ({
              id: value,
              label: compactLabel(thinkingLabel(value)),
            }))}
            current={props.controls.thinking.current}
            currentIcon={
              <Icon name="outline-sliders" class="shrink-0 opacity-60" />
            }
            class="max-w-[140px]"
            onSelect={(value) =>
              props.controls.thinking.select(value as OmpThinkingLevel)
            }
          />
        </div>
      </Show>
      <div
        ...
        classList={{
          "pointer-events-none opacity-50": props.controls.approval.loading,
        }}
      >
        <PromptInputV2Select
          title={language.t("prompt.omp.approval.title")}
          options={(
            ["always-ask", "write", "yolo"] satisfies OmpApprovalMode[]
          ).map((value) => ({
            id: value,
            label: compactLabel(approvalLabel(value)),
          }))}
          current={props.controls.approval.current}
          currentIcon={<Icon name="check" class="shrink-0 opacity-60" />}
          class="max-w-[150px]"
          onSelect={(value) =>
            props.controls.approval.select(value as OmpApprovalMode)
          }
        />
      </div>
    </Show>
...
export function usePromptInputV2Controller(
  props: PromptInputV2ControllerProps,
): PromptInputV2ComposerController {
  const sdk = useSDK();
  const sync = useSync();
  const files = useFile();
  ...
  const recent = createMemo(() => {
    ...
    const order = active
      ? [active, ...all.filter((tab) => tab !== active)]
      : all;
  ...
  const info = createMemo(() =>
    props.controls.session.id
      ? sync().session.get(props.controls.session.id)
      : undefined,
  );
  const working = createMemo(() =>
    sync().data.session_working(props.controls.session.id ?? ""),
  );
  ...
  const commentCount = createMemo(() => {
    ...
    return prompt.context.items().filter((item) => !!item.comment?.trim())
      .length;
  });
  ...
  const historyComments = () => {
    const byID = new Map(
      comments.all().map((item) => [`${item.file}\n${item.id}`, item] as const),
    );
  ...
  const submission = createPromptSubmit({
    ...
    setMode: (next) =>
      controller.dispatch({
        type: next === "shell" ? "mode.shell" : "mode.normal",
      }),
    setPopover: (popover) => {
  ...
  const slashCommands = createMemo(() => [
    ...
    ...command.options
      .filter(
        (item) =>
          !item.disabled && !item.id.startsWith("suggested.") && item.slash,
      )
      .map((item) => ({
        id: item.id,
        trigger: item.slash!,
  ...
  const controller = createPromptInputV2Controller({
    ...
    history: {
      ...
      add: (value, mode) =>
        history.add(value, mode, mode === "shell" ? [] : historyComments()),
      capture: historyComments,
      restore: (metadata) =>
        restoreHistoryComments(metadata as PromptHistoryComment[]),
    },
    commands,
    context,
    ...
    openAttachment: (attachment) =>
      dialog.show(() => (
        <ImagePreview src={attachment.blob.url} alt={attachment.filename} />
      )),
    openContext(key) {
    ...
    attachments: {
      ...
      duplicate: () =>
        showToast({
          title: language.t("prompt.toast.attachmentDuplicate.title"),
        }),
      onError: (error) =>
        showToast({
          variant: "error",
  ...
  Object.defineProperty(controller, "model", {
    get: () => props.controls.model,
  });

  Object.defineProperty(controller, "omp", { get: () => props.controls.omp });
...
function PromptInputV2ModelControl(props: {
  ...
  const shouldAnimate = createMemo<boolean>(
    (previous) => previous ?? props.loading,
  );
  const content = () => (
    <>
  ...
  const queueFocus = (attempts = 6) => {
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        ...
        if (current?.file === focus.file && current.id === focus.id)
          queueFocus(attempts - 1);
      });
    });
  };
  const diffs = props.controls.session.id
    ? sync().data.session_diff[props.controls.session.id]
    : undefined;
  const review =
    item.commentOrigin === "review" ||
    (item.commentOrigin !== "file" &&
      diffs?.some((diff) => diff.file === item.path));
  if (!props.controls.session.reviewPanel.opened())
