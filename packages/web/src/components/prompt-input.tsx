...
} from "@/components/dialog-select-model";
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const scrollCursorIntoView = () => {
    ...
    const length = promptLength(
      prompt.current().filter((part) => part.type !== "image"),
    );
    if (cursor >= length) {
  ...
  const openComment = (item: {
    ...
    void tabs().open(tab);
    tabs().setActive(tab);
    void Promise.resolve(files.load(item.path)).finally(() =>
      queueCommentFocus(),
    );
  };
  ...
  const restoreFocus = () => {
    requestAnimationFrame(() => {
      const cursor =
        savedCursor ?? prompt.cursor() ?? promptLength(prompt.current());
      editorRef.focus();
  ...
  const handleFocus = () => {
    ...
    requestAnimationFrame(() => {
      ...
      setCursorPosition(
        editorRef,
        prompt.cursor() ?? promptLength(prompt.current()),
      );
      queueScroll();
    });
  };
  ...
  const handleBlur = () => {
    const cursor = currentCursor();
    savedCursor = cursor;
    if (cursor !== null && cursor !== prompt.cursor())
      prompt.set(prompt.current(), cursor);
    closePopover();
    setComposing(false);
  };
  ...
  const slashCommands = createMemo<SlashCommand[]>(() => {
    const builtin = command.options
      .filter(
        (opt) => !opt.disabled && !opt.id.startsWith("suggested.") && opt.slash,
      )
      .map((opt) => ({
        id: opt.id,
        trigger: opt.slash!,
  ...
  const scrollSlashActiveIntoView = () => {
    ...
    requestAnimationFrame(() => {
      const element = slashPopoverRef.querySelector(
      ...
      if (last) {
        ...
        if (last.nodeType !== Node.TEXT_NODE) {
          ...
          if (isBreak && (!next || emptyText)) {
            const placeholder =
              next && emptyText ? next : document.createTextNode("\u200B");
  ...
  const { addAttachment, addAttachments, removeAttachment, handlePaste } =
  ...
  const providersShouldFadeIn = createMemo<boolean>(
    (prev) => prev ?? providersLoading(),
  );
      ...
      >
        ...
        <PromptContextItems
          ...
          active={(item) => {
            ...
            return (
              !!item.commentID &&
              item.commentID === active?.id &&
              item.path === active?.file
            );
          }}
              ...
              <div class="flex items-center gap-1.5 min-w-0 flex-1 h-7">
                <Show when={!providersLoading()}>
                  <Show when={store.mode !== "shell"}>
                    ...
                    <Show when={props.controls.omp.visible}>
                      <Show
                        when={props.controls.omp.thinking.options.length > 1}
                      >
                        <Tooltip
                          placement="top"
                          value={language.t("prompt.omp.thinking.title")}
                        >
                          <Select
                            size="normal"
                            options={props.controls.omp.thinking.options}
                            current={props.controls.omp.thinking.current}
                            label={(value) =>
                              language.t(
                                `prompt.omp.thinking.${value}` as `prompt.omp.thinking.${OmpThinkingLevel}`,
                              )
                            }
                            onSelect={(value) => {
                              props.controls.omp.thinking.select(
                                value as OmpThinkingLevel,
                              );
                              restoreFocus();
                            }}
                            disabled={props.controls.omp.thinking.loading}
                            class="capitalize max-w-[140px] text-text-base"
                            valueClass="truncate text-13-regular text-text-base"
                            triggerStyle={control()}
                            triggerProps={{ "data-action": "prompt-thinking" }}
                            variant="ghost"
                          />
                        </Tooltip>
                      </Show>
                      <Tooltip
                        placement="top"
                        value={language.t("prompt.omp.approval.title")}
                      >
                        <Select
                          size="normal"
                          options={["always-ask", "write", "yolo"]}
                          current={props.controls.omp.approval.current}
                          label={(value) => {
                            const labels: Record<OmpApprovalMode, string> = {
                              "always-ask": language.t(
                                "prompt.omp.approval.alwaysAsk",
                              ),
                              write: language.t("prompt.omp.approval.write"),
                              yolo: language.t("prompt.omp.approval.yolo"),
                            };
                            return labels[value as OmpApprovalMode];
                          }}
                          onSelect={(value) => {
                            props.controls.omp.approval.select(
                              value as OmpApprovalMode,
                            );
                            restoreFocus();
                          }}
                          disabled={props.controls.omp.approval.loading}
