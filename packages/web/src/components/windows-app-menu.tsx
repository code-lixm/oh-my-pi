...
export function WindowsAppMenu(props: {
  ...
  const entryDisabled = (entry: DesktopMenuEntry) => {
    if (entry.type === "separator") return false
    if (entry.enabled === "updater") {
      const state = props.platform.updater?.state()
      if (!state || state.status === "disabled") return true
    }
    return entry.command ? commandDisabled(entry.command) : false
  }
  const runEntry = (entry: DesktopMenuEntry) => {
    if (entry.type === "separator" || entryDisabled(entry)) return
    if (entry.command) {
    ...
    <DropdownMenu gutter={4} modal={false} placement="bottom-start">
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content class="desktop-app-menu">
          <DropdownMenu.Group>
            <DropdownMenu.GroupLabel class="desktop-app-menu-heading">OMP</DropdownMenu.GroupLabel>
