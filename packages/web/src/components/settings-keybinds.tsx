...
const IconV2 = lazy(() =>
  import("@opencode-ai/ui/v2/icon").then((module) => ({
    default: module.Icon,
  })),
);
...
const RETIRED_WEB_COMMANDS = new Set(["agent.cycle", "agent.cycle.reverse"]);
...
function listFor(
  ...
  for (const opt of command.catalog) {
    if (RETIRED_WEB_COMMANDS.has(opt.id)) continue;
  ...
  for (const [id, value] of Object.entries(map)) {
    if (RETIRED_WEB_COMMANDS.has(id)) continue;
...
function groupedFor(list: Map<string, KeybindMeta>) {
  ...
  for (const group of GROUPS) {
    ...
    if (!ids) continue;
    ids.sort((a, b) =>
      (list.get(a)?.title ?? "").localeCompare(list.get(b)?.title ?? ""),
    );
  }
...
export function createKeybindSettingsController(
  ...
  const effective = (id: string) => {
    if (id === PALETTE_ID)
      return input.settings.keybinds.get(id) ?? DEFAULT_PALETTE_KEYBIND;
  ...
  const notify =
    input.notify ??
    ((toast: { title: string; description: string }) => showToast(toast));
  ...
  const target =
    input.target ?? (typeof document === "object" ? document : undefined);
  ...
  return {
    catalog: {
      ...
      filtered: (query: string) =>
        filteredFor(query, list(), grouped(), (id) =>
          formatKeybind(effective(id) ?? "", language.t),
        ),
      title,
...
function SettingsKeybindsV2View(props: {
  ...
  const hasResults = createMemo(() =>
    props.groups.some((group) => (filtered().get(group)?.length ?? 0) > 0),
  );
        ...
        <div class="settings-v2-shortcuts flex flex-col gap-8">
          ...
          <Show when={store.filter && !hasResults()}>
            <div class="settings-v2-shortcuts-status">
              ...
              <span class="settings-v2-shortcuts-status-filter">
                &quot;{store.filter}&quot;
              </span>
            </div>
          </Show>
...
export const SettingsKeybinds: Component<{ v2?: boolean }> = (props) => {
  ...
  const used = createMemo(() => {
    ...
    const palette =
      settings.keybinds.get(PALETTE_ID) ?? DEFAULT_PALETTE_KEYBIND;
