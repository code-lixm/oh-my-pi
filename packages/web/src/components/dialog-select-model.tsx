...
import { DialogManageModels, DialogManageModelsV2 } from "./dialog-manage-models"
          ...
          <ModelList
            ...
            action={
              <div class="flex items-center gap-1">
                <Show when={capabilities()?.providerWrite === true}>
  ...
  return {
    models: (search: string) => {
      ...
      const filtered = query
        ? available.filter((item) => matchesModelSearch(query, [item.name, item.id, item.provider.name]))
        : available
