...
import type { AssistantMessage, FilePart, Message, Part, ToolPart, UserMessage } from "@/omp/types"
...
function toolPart(sessionID: string, messageID: string, tool: SessionMessageAssistantTool): ToolPart {
  ...
  const state = (() => {
    if (tool.state.status === "streaming") {
      ...
      const input = normalizeToolInput(tool.name, value)
    ...
    if (tool.state.status === "running") {
      return {
        ...
        metadata: normalizeToolMetadata(tool.name, tool.state.metadata ?? {}, tool.state.input),
        time: { start },
      }
    }
