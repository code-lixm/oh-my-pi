...
import { OMP_WEB_PRODUCT, OMP_WEB_PROTOCOL } from "../../shared/omp-web-contract"
...
export async function checkServerHealth(
  ...
  const attempt = async (count: number): Promise<ServerHealth> => {
    return fetch(new URL("/api/health", server.url), {
      signal,
      ...
      .then(async (response): Promise<ServerHealth> => {
        if (!response.ok) return { healthy: false }
        const value: unknown = await response.json()
        if (!value || typeof value !== "object") return { healthy: false }
        if (!("healthy" in value) || value.healthy !== true) return { healthy: false }
        if (!("product" in value) || value.product !== OMP_WEB_PRODUCT) return { healthy: false }
        if (!("protocol" in value) || value.protocol !== OMP_WEB_PROTOCOL) return { healthy: false }
        return { healthy: true, version: "version" in value && typeof value.version === "string" ? value.version : undefined }
      })
