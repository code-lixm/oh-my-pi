...
const GLOBAL_STORAGE = "omp.global.dat"
const WINDOW_STORAGE = "omp.window"
const LOCAL_PREFIX = "omp."
...
async function migrateLegacyAsync(input: {
  ...
  for (const key of input.keys) {
    const raw = await input.current.getItem(key)
    if (raw === null) continue

    const next = normalize(input.defaults, raw, input.migrate)
    if (next === undefined) {
      await removeAsync(input.current, key)
      continue
    }
    await input.current.setItem(input.key, next)
    await input.current.removeItem(key)
    return next
  }
...
function workspaceStorage(dir: string) {
  ...
  return `omp.workspace.${head}.${sum}.dat`
}
...
export function removePersisted(
  ...
  const storage = target.storage ?? GLOBAL_STORAGE
  if (target.draft && platform?.draftStore) {
    void platform.draftStore.removeItem(`${storage}:${target.key}`)
  }
  const isDesktop = platform?.platform === "desktop" && !!platform.storage

  if (isDesktop) {
    void platform.storage?.(storage)?.removeItem(target.key)
    for (const legacyStorage of target.legacyStorageNames ?? []) {
      void platform.storage?.(legacyStorage)?.removeItem(target.key)
    }
    return
  }
  ...
  for (const legacyStorage of target.legacyStorageNames ?? []) {
    localStorageWithPrefix(legacyStorage).removeItem(target.key)
  }
}
...
export function persisted<T>(
  ...
  const input = typeof target === "string" ? { key: target } : target
  const config = resolveTarget({ ...input, storage: input.storage ?? GLOBAL_STORAGE }, platform)
  const storageName = config.storage ?? GLOBAL_STORAGE
  ...
  const storage = (() => {
    ...
    const oldCurrent = draft
      ? isDesktop
        ? platform.storage?.(storageName)
        : localStorageWithPrefix(storageName)
      : undefined
    const legacyStores = [
      oldCurrent,
