...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("omp.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("omp.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("omp.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("omp.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("omp.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("omp.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("omp.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("omp.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("omp.safe.scope:value")).toBe('{"value":3}')
  })

  test("does not use unprefixed direct storage as a legacy fallback", () => {
    ...
    expect(storage.getItem("legacy.workspace")).toBe('{"value":2}')
  })
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("omp.workspace.")
    expect(result.endsWith(".dat")).toBeTrue()
    expect(/[:\\/]/.test(result)).toBeFalse()
  })

  test("window storage uses the OMP namespace", () => {
    expect(persistTesting.windowStorage("window-1")).toBe("omp.window.window-1.dat")
  })
  ...
  test("migrates legacy keys within scoped OMP storage", () => {
    const target = Persist.workspace("C:/Users/foo", "demo", ["legacy.workspace"])
    const current = persistTesting.localStorageWithPrefix(target.storage!)
    current.setItem("legacy.workspace", '{"value":2}')
    ...
    expect(storage.getItem(`${target.storage}:legacy.workspace`)).toBeNull()
  })
  ...
  test("draft target isolates storage per draft and namespaces keys", () => {
    ...
    expect(a.storage).toStartWith("omp.draft.")
