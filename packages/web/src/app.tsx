...
import { OmpSettingsProvider } from "@/components/settings-v2/omp-settings-context"
...
const SessionRoute = () => {
  ...
  // is replaced by a draft at /new-session?draftId=…. Do not mount SessionPage while the
  // draft is being created: its directory-scoped data is not ready on this transient route.
  const redirectingToDraft = createMemo(() => settings.general.newLayoutDesigns() && !params.id && !search.draftId)
  const [draftError, setDraftError] = createSignal<Error>()
  let draftStarted = false
  createEffect(() => {
    if (!redirectingToDraft() || draftStarted) return
    if (!tabs.ready() || !sdk().directory) return
    draftStarted = true
    void tabs
      .newDraft({ server: server.key, directory: sdk().directory }, search.prompt)
      .catch((error) => setDraftError(error instanceof Error ? error : new Error(String(error))))
  })
...
function ScopedServerProviders(props: { server?: ServerConnection.Any; children: () => JSX.Element }) {
  return (
    <ServerSDKProvider server={props.server}>
      <ServerSyncProvider server={props.server}>{props.children()}</ServerSyncProvider>
    </ServerSDKProvider>
  )
}

function TargetServerRoute(props: { children: () => JSX.Element }) {
  ...
  return (
    ...
    <Show when={conn()} keyed>
      {(connection) => <ScopedServerProviders server={connection} children={props.children} />}
    </Show>
  )
}

const TargetSessionRoute = () => <TargetServerRoute>{() => <TargetSessionRouteContent />}</TargetServerRoute>
...
function SelectedServerProviders(props: { children: () => JSX.Element }) {
  return (
    <ServerKey>{(connection) => <ScopedServerProviders server={connection} children={props.children} />}</ServerKey>
  )
}
...
function ResolvedDraftRoute(props: { draft: DraftTab }) {
  ...
  return (
    <Show when={conn()} keyed>
      {(connection) => (
        <ScopedServerProviders server={connection}>
          {() => (
...
function setNativeTitlebar(theme: { mode: "light" | "dark"; scheme?: "system" | "light" | "dark" }) {
  if (!("api" in window)) return
  const api = window.api
  if (!api || typeof api !== "object" || !("setTitlebar" in api) || typeof api.setTitlebar !== "function") return
...
function ServerScopedProviders(props: ServerScopedShellProps) {
  ...
  return (
    <LayoutProvider>
      <OmpSettingsProvider directory={directory}>
        {props.serverScoped}
...
function ConnectionGate(props: ParentProps<{ disableHealthCheck?: boolean; startup?: Promise<void> }>) {
  ...
  const checking = createMemo(
    () =>
      !props.disableHealthCheck &&
      checkMode() === "blocking" &&
      ["unresolved", "pending"].includes(startupHealthCheck.state),
  )
  ...
  const startupChecking = createMemo(
    () => !!props.startup && startupHealthCheck.latest === true && ["unresolved", "pending"].includes(startup.state),
  )
...
function ServerKey(props: { children: (server: ServerConnection.Any) => JSX.Element }) {
  const server = useServer()
  return (
