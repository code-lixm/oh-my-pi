...
const domains = [
  ...
  {
    name: "ui",
    source: "../../vendor/opencode/ui/src/i18n/en.ts",
    target: (locale: string) => `../../vendor/opencode/ui/src/i18n/${locale}.ts`,
    locales: appLocales,
  },
  {
    name: "desktop",
...
describe("i18n parity", () => {
  ...
  test("changed-file summary keys preserve rendered English copy and localize complete phrases", async () => {
    const source = await dictionary("../../vendor/opencode/ui/src/i18n/en.ts")
    ...
    for (const locale of appLocales) {
      const target = await dictionary(`../../vendor/opencode/ui/src/i18n/${locale}.ts`)
