...
import { Message, Part, UserMessage } from "@/omp/types"
...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const toggleAutoAccept = () => {
    ...
    if (!directory) return
    const sessionID = params.id
    if (sessionID) permission.toggleAutoAccept(sessionID, directory)
    else permission.toggleAutoAcceptDirectory(directory)
  ...
  const shareCmds = () => {
    if (!capabilities()?.sessionShare) return []
    const sharing = sync().data.config.share
    if (sharing !== "manual" && sharing !== "auto") return []
    return [
      sessionCommand({
        id: "session.share",
