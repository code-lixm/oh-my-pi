...
import { useServerCapabilities, useServerSDK } from "@/context/server-sdk"
...
export default function LegacyLayout(props: ParentProps) {
  ...
  const workspaceSetting = createMemo(() => {
    if (!capabilities()?.workspaceWrite) return false
  ...
  async function archiveSession(session: Session) {
    if (!capabilities()?.sessionArchive) return
  ...
  function connectProvider() {
    if (!capabilities()?.providerWrite) return
    const run = ++dialogRun
  ...
  async function renameProject(project: LocalProject, next: string) {
    if (!capabilities()?.projectMetadataWrite) return
  ...
  const renameWorkspace = (directory: string, next: string, projectId?: string, branch?: string) => {
    if (!capabilities()?.workspaceWrite) return
  ...
  const resetWorkspace = async (root: string, directory: string) => {
    ...
    if (capabilities()?.sessionArchive)
      await Promise.all(
        sessions
  ...
  const projectSidebarCtx: ProjectSidebarContext = {
    ...
    workspacesEnabled: (project) =>
      capabilities()?.workspaceWrite === true && project.vcs === "git" && layout.sidebar.workspaces(project.worktree)(),
    workspaceIds,
    workspaceLabel,
    sessionProps: {
      navList: currentSessions,
  ...
  const SidebarPanel = (panelProps: {
    ...
    const workspaces = createMemo(() => {
      ...
      return capabilities()?.workspaceWrite ? workspaceIds(item) : [item.worktree]
    })
    ...
    return (
      ...
      >
          ...
          {(project) => (
            <>
              <div class="shrink-0 pl-1 py-1">
                <div class="group/project flex items-start justify-between gap-2 py-2 pl-2 pr-0">
                  <div class="flex flex-col min-w-0">
                    <Show
                      when={capabilities()?.projectMetadataWrite}
                  ...
                  <DropdownMenu modal={!sidebarHovering()}>
                    ...
                    <DropdownMenu.Portal>
                      <DropdownMenu.Content class="mt-1">
                        <Show when={capabilities()?.projectMetadataWrite}>
                        <DropdownMenu.Item
                          onSelect={() => {
                        ...
                        <Show when={capabilities()?.workspaceWrite}>
                        <DropdownMenu.Item
              ...
              <div class="flex-1 min-h-0 flex flex-col">
                <Show
                  when={capabilities()?.workspaceWrite === true && workspacesEnabled()}
                  fallback={
                    <>
        ...
        <div
          ...
          classList={{
            hidden:
              capabilities()?.providerWrite !== true ||
          ...
          <div class="rounded-xl bg-background-base shadow-xs-border-base" data-component="getting-started">
            <div class="p-3 flex flex-col gap-6">
              ...
              <div data-component="getting-started-actions">
                <Show when={capabilities()?.providerWrite}>
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openExternal("https://omp.sh")}
      renderPanel={() =>
