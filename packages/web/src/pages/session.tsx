import type { FilePart, Project, UserMessage, VcsFileDiff } from "@/omp/types"
...
function TargetSessionPage() {
  ...
  const identity = () => {
    try {
      ...
      return scope && directory ? `${scope}\0${directory}` : ""
    } catch {
      return ""
    }
  }
...
export default function Page() {
  ...
  const newSessionWorktree = createMemo(() => {
    if (!capabilities()?.workspaceWrite) return "main"
  ...
  const actions = createMemo(() => (capabilities()?.sessionRevert ? { revert, openAttachment } : { openAttachment }))
          ...
          const controller = createSessionComposerRegionController({
            ...
            followup: () =>
              params.id && !isChildSession()
                ...
                : undefined,
            revert: () =>
              capabilities()?.sessionRevert === true && rolled().length > 0
                ? {
                    items: rolled(),
                    restoring: restoring(),
                    disabled: reverting(),
