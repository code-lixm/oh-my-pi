...
import { type Session } from "@/omp/types"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("omp://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("rejects invalid deep-link hosts and schemes", () => {
    expect(parseDeepLink("omp://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://open-project?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("opencode://open-project?directory=/tmp/demo")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("omp://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("omp://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("omp://open-project")).toBeUndefined()
    expect(parseDeepLink("omp://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "omp://open-project?directory=/a",
      "omp://other?directory=/b",
      "omp://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("omp://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("omp://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("omp://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("omp://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "omp://new-session?directory=/a",
      "omp://open-project?directory=/b",
      "omp://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OMP__: {
        deepLinks: ["omp://open-project?directory=/a"],
      },
    } as unknown as Window & { __OMP__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["omp://open-project?directory=/a"])
