...
export const ErrorPage: Component<ErrorPageProps> = (props) => {
  ...
  function ensureFatalErrorRecorded() {
    recordedFatalError ??=
      platform.recordFatalRendererError?.({
        ...
        url: location.href,
        version: platform.runtimeVersion?.(),
  ...
  const updater = () => (platform.updater?.state().status === "disabled" ? undefined : platform.updater)
  ...
  return (
    <div class="relative flex-1 h-screen w-screen min-h-0 overflow-y-auto font-sans" data-tauri-drag-region>
      <div class="mx-auto min-h-full w-full max-w-3xl flex flex-col items-center justify-center gap-8 px-6 py-12 sm:py-16">
              ...
              <Show
                ...
                fallback={
                  <Button
                    ...
                    disabled={["checking", "downloading", "installing"].includes(currentUpdater().state().status)}
                  >
                    {currentUpdater().state().status === "checking"
        ...
        <Show when={platform.runtimeVersion?.()}>
          {(version) => (
