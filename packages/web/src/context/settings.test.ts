...
describe("shell tool expansion migration", () => {
  test("expands Shell results once for legacy profiles missing the migration marker", () => {
    expect(initialShellToolExpansion(undefined)).toBe(true)
  })

  test("keeps a user's Shell result expansion choice after the migration marker is recorded", () => {
    expect(initialShellToolExpansion(true)).toBeUndefined()
  })
})
