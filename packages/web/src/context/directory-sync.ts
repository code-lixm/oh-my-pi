...
import type { Message, Part, Session } from "@/omp/types"
...
export const createDirSyncContext = (
  ...
  const current = () => serverSync.child(directory, { mcp: true })
  ...
  return {
    ...
    session: {
      ...
      archive: async (sessionID: string) => {
        if (serverSDK.capabilityKind()?.sessionArchive !== true) return
