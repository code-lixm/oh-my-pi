...
import type { OpencodeClient } from "@/omp/types"
import type { SessionApi, SessionInfo, SessionListInput } from "@/omp/api"
  ...
  test("loads current servers for the requested location", async () => {
    ...
    const result = await queryClient.fetchQuery(
      loadMcpQuery(ServerScope.local, "/project", {
        runtime: {
          mcpStatuses: async (input?: { directory?: string }) => {
            calls.push(input)
            return { docs: { status: "connected" }, search: { status: "pending" } }
          },
        },
      } as unknown as Pick<OmpWebApi, "runtime">),
    )

    expect(calls).toEqual([{ directory: "/project" }])
  ...
  test("loads and keys the current resource catalog", async () => {
    ...
    const result = await queryClient.fetchQuery(
      loadMcpResourcesQuery(ServerScope.local, "/project", {
        runtime: {
          mcpResources: async (input?: { directory?: string }) => {
            calls.push(input)
            return { "docs:docs://guide": { server: "docs", name: "Guide", uri: "docs://guide" } }
          },
        },
      } as unknown as Pick<OmpWebApi, "runtime">),
    )
