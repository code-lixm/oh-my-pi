...
import type { Message, Part } from "@/omp/types"
...
export const useSync = () => {
  ...
  const sdk = useSDK()
  const initialServer = serverSync()
  const initialSDK = sdk()
  if (!initialServer || !initialSDK) return createMemo(() => serverSync().ensureDirSyncContext(sdk().directory))
  const initial = initialServer.ensureDirSyncContext(initialSDK.directory)

  return createMemo(() => {
    const nextServer = serverSync()
    const nextSDK = sdk()
    if (!nextServer || !nextSDK) return initial
    return nextServer.ensureDirSyncContext(nextSDK.directory)
  }, initial)
}
