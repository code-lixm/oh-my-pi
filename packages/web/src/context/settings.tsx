...
export interface Settings {
  general: {
    ...
    shellToolExpansionInitialized?: boolean
...
const legacyNewLayoutDesignsDefault = import.meta.env.VITE_OMP_CHANNEL !== "prod"
...
export function initialShellToolExpansion(initialized: boolean | undefined) {
  if (initialized === true) return
  return true
}
...
const defaultSettings: Settings = {
  general: {
    ...
    shellToolExpansionInitialized: false,
...
export const { use: useSettings, provider: SettingsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    createEffect(() => {
      if (!ready()) return
      const initial = initialShellToolExpansion(store.general?.shellToolExpansionInitialized)
      if (initial === undefined) return
      batch(() => {
        setStore("general", "shellToolPartsExpanded", initial)
        setStore("general", "shellToolExpansionInitialized", true)
      })
    })
