...
import type { Message, OpencodeClient, Part, Session } from "@/omp/types"
...
describe("server session", () => {
  ...
  test("indexes OMP transport messages for the timeline", async () => {
    ...
    expect(
      store.data.session_message.root.map((message) =>
        message.type === "user"
  ...
  test("keeps a root assistant without backfilling an empty parent", async () => {
    const assistant = assistantMessage("message-2", "")
    const client = rootMessageClient(
      [response([{ info: assistant, parts: [] }], "older")],
      [singleResponse(userMessage("message-1"))],
    )
    const store = createServerSession(client)
  ...
  test("does not scan cached messages for user roots during history prepend", async () => {
    ...
    expect(store.data.message.child.map((message) => message.id)).toEqual([older.id, latest.id])
  })
