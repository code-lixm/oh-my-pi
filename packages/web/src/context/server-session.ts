...
import type { SessionMessageInfo } from "@/omp/api"
import type {
  Message,
  Part,
  PermissionRequest,
...
import type { FileDiffInfo } from "@/omp/api"
...
function transportMessageSource(items: { info: Message; parts: Part[] }[]): SessionMessageInfo[] {
  return items
    .slice()
...
export function createServerSession(client: OmpTransportClient, options?: ServerSessionOptions) {
  ...
  const indexTransportMessage = (message: Message) => {
    const current = data.session_message[message.sessionID] ?? []
    if (current.some((item) => item.id === message.id)) return
    setData(
      "session_message",
      message.sessionID,
      reconcile([...current, ...transportMessageSource([{ info: message, parts: [] }])]),
    )
  }
  ...
  const fetchMessages = async (sessionID: string, limit: number, before?: string, onAttempt?: () => void) => {
    ...
    return {
      ...
      source: transportMessageSource(items),
