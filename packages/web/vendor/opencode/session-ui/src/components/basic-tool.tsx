...
import { formatToolRecord, shouldDeferGenericToolContent } from "./tool-details"
...
function present(value: Record<string, unknown> | undefined) {
...
export function GenericTool(props: {
  ...
  return (
    <BasicTool
      ...
      hideDetails={props.hideDetails || (!present(props.input) && !present(props.metadata) && !props.output)}
      defaultOpen={props.defaultOpen}
      open={props.open}
      onOpenChange={props.onOpenChange}
      allowOpenWhilePending
      defer={shouldDeferGenericToolContent(props.deferContent, props.open)}
    >
      <div data-component="generic-tool-details">
        <Show when={present(props.input)}>
          <section data-slot="generic-tool-section">
            <div data-slot="generic-tool-label">{i18n.t("ui.basicTool.input")}</div>
            <pre data-slot="generic-tool-value">{formatToolRecord(props.input)}</pre>
          </section>
        </Show>
        <Show when={props.output}>
          {(output) => (
            <section data-slot="generic-tool-section">
              <div data-slot="generic-tool-label">{i18n.t("ui.basicTool.output")}</div>
              <pre data-slot="generic-tool-value">{output()}</pre>
            </section>
          )}
        </Show>
        <Show when={present(props.metadata)}>
          <section data-slot="generic-tool-section">
            <div data-slot="generic-tool-label">{i18n.t("ui.basicTool.metadata")}</div>
            <pre data-slot="generic-tool-value">{formatToolRecord(props.metadata)}</pre>
          </section>
        </Show>
      </div>
    </BasicTool>
