...
export function renderable(part: PartType, showReasoningSummaries = true) {
  return isPartRenderable(part, showReasoningSummaries, (type) => !!PART_MAPPING[type])
}
...
function isContextGroupTool(part: PartType): part is ToolPart {
  return part.type === "tool" && isContextGroupToolName(part.tool)
}
...
export function UserMessageDisplay(props: {
  ...
  const textParts = createMemo(
    () => props.parts?.filter((part): part is TextPart => part.type === "text" && !part.synthetic) ?? [],
  )
  const textPart = createMemo(() => textParts()[0])
  const text = createMemo(() => visibleUserMessageText(textParts()))
...
PART_MAPPING["tool"] = function ToolPartDisplay(props) {
  ...
  const presentationName = createMemo(() => presentationToolName(part().tool, input()))
  const usesXdTransport = createMemo(() => isXdToolTransport(part().tool, input()))
  ...
  const render = createMemo(() => {
    if (usesXdTransport()) return GenericTool
    if (part().tool === "edit" && patchFiles(partMetadata().files).length > 1) {
      return ToolRegistry.render("patch") ?? GenericTool
    }
...
type OmpCompactionDisplayPart = CompactionPart & {
  summary?: string
  warning?: string
  tokensBefore?: number
}
...
PART_MAPPING["compaction"] = function CompactionPartDisplay(props) {
  const i18n = useI18n()
  const part = () => props.part as OmpCompactionDisplayPart
  const text = createMemo(() => compactionDisplayText(part().summary ?? "", part().warning))

  return (
    <div data-component="compaction-detail">
      <MessageDivider label={i18n.t("ui.messagePart.compaction")} />
      <Show when={text()}>
        <div data-slot="compaction-part-summary">
          <PacedMarkdown text={text()} cacheKey={`${part().id}:summary`} streaming={false} />
        </div>
      </Show>
    </div>
  )
        ...
        fallback={
          <div data-component="apply-patch-tool">
            <BasicTool
              ...
              defer={props.deferContent !== false}
              trigger={{
                title: i18n.t(props.tool === "edit" ? "ui.messagePart.title.edit" : "ui.tool.patch"),
                subtitle: subtitle(),
              }}
            >
              ...
              <div data-component="edit-trigger">
                <div data-slot="message-part-title-area">
                  <div data-slot="message-part-title">
                    <span data-slot="message-part-title-text">
                      <TextShimmer
                        text={i18n.t(props.tool === "edit" ? "ui.messagePart.title.edit" : "ui.tool.patch")}
                        active={pending()}
                      />
                    </span>
                    <Show when={!pending()}>
