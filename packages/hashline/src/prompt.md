...
<critical>
1. RE-GROUND AFTER EVERY EDIT — applied edits renumber the file and change the `#TAG`; take next numbers from the edit response or a fresh current snapshot. Stale tag or surprise? STOP, `read` the affected range; NEVER rerun CodeGraph only to refresh a snapshot.
