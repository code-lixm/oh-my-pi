...
| Worker-host dispatch | CLI worker selectors, source/SDK fallbacks, tiny worker environment, system/user chat-template completion prompts, browser-tab startup, stats sync, and compiled-binary subprocesses stay synchronized. | `test/worker-selector.test.ts`, `test/tiny-worker-env.test.ts`, `test/tiny-title-generator.test.ts`, `test/tools/browser-tab-worker-startup.test.ts`, `omp --smoke-test` |
| OMP-native Web parity | HTTP, SSE, and restored history share canonical OMP view models; multi-part text, user/Assistant/tool-result images, compaction/branch summaries, interrupted turns, and every OMP tool preserve their visible content plus expandable input/output/error/details; hidden custom events stay hidden; Web controls map to live RPC runtime, sessions, models, commands, checkpoints, subagents, jobs, settings, keybindings, MCP, and plugins without OpenCode fallback behavior. | `packages/web/server/bridge-contract.test.ts`, `packages/web/server/projection.test.ts`, `packages/web/vendor/opencode/session-ui/src/components/message-part.test.ts`, `packages/web/vendor/opencode/session-ui/src/components/tool-visibility.test.ts`, `packages/web/vendor/opencode/session-ui/src/components/tool-details.test.ts`, `test/rpc-input-frame.test.ts`, `test/rpc-message-transcript.test.ts`, `test/rpc-client.restart.test.ts` |
...
| Retry/fallback lifecycle | Each endpoint exhausts `retry.maxRetries`; temporary model fallbacks probe and safely restore primaries without persistence leaks; model switches cancel pending recovery; one retry saga persists a single collapsed empty-error diagnostic row (the terminal cause keeps its own row). | `test/agent-session-retry-fallback.test.ts`, `test/agent-session-retry-recovery.test.ts`, `test/retry-fallback.test.ts` |
| Retry settings/model keys | Arbitrary non-negative retry limits validate; Alt+P/Ctrl+Alt+P cycle model roles (Tab keeps the composer's native completion accept/trigger, including while the completion list is open); Ctrl+P cycles thinking without autocomplete conflicts. | `test/modes/components/settings-selector-retry-attempts.test.ts`, `test/agent-session-role-thinking.test.ts`, `test/input-controller-keybindings.test.ts`, `test/custom-editor-keybindings.test.ts`, `test/keybindings-display.test.ts` |
...
| Recoverable context pruning | Automatic pruning archives data-bearing outputs behind durable `artifact://` placeholders, fails closed on archive loss, deduplicates only exact operation results, removes only proven-resolved errors outside the recent window, adapts budgets to the model window with a user-overridable token protection window, and carries deterministic execution facts across compaction without altering transcripts. | `packages/agent/test/supersede-prune.test.ts`, `test/agent-session-prune-persistence.test.ts`, `test/compaction-retained-facts.test.ts` |
...
| Native PDF reading | PDF reads use the `pdf-inspector` native binding, preserve Markdown caching plus line/multi-range/raw selectors, report pages needing OCR, render legacy PDF image-member paths as browser-owned page screenshots with bounded cleanup, and keep MuPDF WASM/package code out of shipped binaries. | `test/pdf-converter.test.ts`, `test/tools/read-pdf-rendering.test.ts`, `packages/natives/test/native.test.ts` |
| FFF indexed search | Session-shared FFF indexes expose only `grep`, `find`, and `multi_grep`; cursor pagination, smart-case/regex/fuzzy content search, frecency/Git-ranked path search, bounded external-path indexes, lifecycle disposal, compiled-binary loading, localized compact renderers, and clean-cut removal of legacy `glob`/`search` surfaces remain intact. | `test/tools/fff-tools.test.ts`, `test/tools/fff-renderer.test.ts`, `test/cursor-exec.test.ts`, `scripts/ci-release-build-binaries.test.ts`, `omp --smoke-test` |
| Native external tool discovery | Essential `tool_search` indexes every registered external tool (including `defaultInactive`) and only external tools, keeps the default prompt to counts instead of catalog summaries, BM25-ranks capability queries with bounded promotion, promotes exact names as top-level schemas for the next model response, rejects every external write/Cursor dispatch and every unpromoted Eval bridge call, and leaves existing built-in load tiers unchanged. | `test/sdk-tool-activation.test.ts`, `test/write-xdev-dispatch.test.ts`, `test/core/js-tool-bridge.test.ts`, `test/issue-5764-registertool-loadmode.test.ts`, `test/system-prompt.test.ts`, `test/system-prompt-model.test.ts` |
| Compact search results | FFF Grep/Find/Multi Grep and CodeGraph keep localized counts, adjacent path trees, one middle omission row, truncation in headers, and zero-result status surfaces. | `test/tools/fff-renderer.test.ts`, `test/tools/codegraph-renderer.test.ts` |
...
| Status-line extensibility | Usage adapters, balances, battery/half-height modes, active-model invalidation, custom presets, basename-only paths, and localized `off`/`percentage`/`annotated`/`embedded` context gauges with real compaction boundaries remain stable. | `test/status-line-usage.test.ts`, `test/status-line-usage-refresh.test.ts`, `test/status-line-path.test.ts`, `test/status-line-settings-cache.test.ts`, `test/status-line-context-cache.test.ts` |
| Skill-prompt session titles | User-invoked skill prompts title sessions from their compact command/chip identity rather than expanded skill bodies, and persisted skill prompts remain valid title-regeneration turns. | `test/session/skill-title-input.test.ts`, `test/session/title-index.test.ts` |
| Startup composer and layouts | Interactive launches prepaint one editable composer and transfer the same editor/draft into the live session; `composer.shape` preserves box, Claude-rule, pi-rule, borderless, top-rule dock, compact field, accent rail, extension-defined layouts, status placement, settings previews, and setup-wizard selection. | `test/startup-composer.test.ts`, `test/startup-composer-graph.test.ts`, `test/composer-cache.test.ts`, `test/modes/components/composer-shape-preview.test.ts`, `test/setup-wizard.test.ts`, `packages/tui/test/editor.test.ts` |
| Composer attachment chips | Pasted images and large text stage as bounded cards above the editor; compact inline tokens remain the submission identity, token deletion removes payloads, surviving image references renumber densely, and queued/restored drafts preserve payload/link ordering. | `test/modes/components/attachment-chips.test.ts`, `test/modes/components/custom-editor.test.ts`, `test/input-controller-escape.test.ts` |
| Image URL broker | Opt-in image publication keeps ordered backend routing, provider-file caches, authenticated and anonymous uploaders, tunnel lifecycle, resilience/fallback, savings accounting, provider-native/remote URL encoding, and safe status/doctor/probe/purge CLI behavior. | `test/blob-broker.test.ts`, `test/blob-provider-fallback.test.ts`, `test/blob-resilience.test.ts`, `test/blob-exposure-tunnels.test.ts`, `test/provider-files.test.ts`, `test/images-cli.test.ts`, `packages/ai/test/image-url-parts.test.ts` |
| Operational TUIs and CLIs | `omp git`/`/git`, `omp ps`, `omp render`, live boards, and related command metadata preserve interactive navigation, process isolation, bounded rendering, and machine-readable non-interactive paths. | `test/live-board.test.ts`, `test/images-cli.test.ts`, `test/cli-command-metadata.test.ts`, `omp --smoke-test` |
| Edit parse-regression safety | Edit parsing accepts sloppy/unified/common malformed variants without corrupting anchors; parse regressions warn, optional auto-repair must reparse before commit, and blackbox capture remains opt-in and evidence-bearing. | `test/edit-blackbox.test.ts`, `test/edit-mode.test.ts`, `test/edit-auto-generated-regressions.test.ts`, `test/edit-streaming-preview.test.ts` |
| macOS prompt assistance | Typo detection, spelling suggestions, word completion, and opt-in autocorrect remain asynchronous, range-safe, configurable, and harmless on unsupported platforms. | `test/macos-spelling.test.ts`, `packages/tui/test/editor.test.ts`, `test/modes/components/custom-editor.test.ts` |
| Extension control center | The fullscreen extension dashboard keeps tabbed keyboard/mouse navigation, normalized inspector models, live tool sessions, MCP runtime actions, expandable details, and source/type/status parity. | `test/extension-dashboard-state.test.ts`, `test/extension-inspector.test.ts`, `test/extension-mcp-inspector.test.ts`, `test/live-tool-session.test.ts` |
| Mouse and rich content | Opt-in mouse input routes editor and Markdown/image hits on the main screen; fullscreen overlays always capture the wheel so terminal alternate-scroll cannot convert it to arrow keys; disabled mode preserves native selection and scrollback in the main transcript; text selection inside fullscreen panels stays reachable via the terminal's shift-drag. | `packages/tui/test/mouse-routing.test.ts`, `packages/tui/test/mouse.test.ts`, `test/modes/components/session-selector-mouse.test.ts`, `test/extension-list-mouse.test.ts`, `test/modes/components/checkpoint-selector.test.ts` |
...
| Tail-first transcript restore | Idle resume/session replacement stages without partial paints, shows the latest viewport through state-isolated tail rendering, then performs one authoritative history replay; streaming rebuilds remain atomic. | `packages/tui/test/resize-viewport-defer.test.ts`, `test/modes/utils/render-initial-messages.test.ts` |
...
| Interrupted activity row | A confirmed double-Esc interrupt freezes the activity row immediately — spinner stopped and `Interrupted.` shown — instead of waiting for the 1.5s teardown grace period; the next submission resumes the row. | `test/interactive-mode-interrupt-loader-freeze.test.ts` |
| Interrupt queue semantics | A user interrupt (double-Esc) is a hard stop for automatic continuation: kept steer/follow-up entries wait for an explicit resume (empty submit, `.`/`c`, next prompt) instead of auto-draining; a localized hint reports the kept count; advisor-steer reclaim and IRC wake parking stay intact. | `test/agent-session-queued-steer-delivery.test.ts`, `test/agent-session-advisor-suppression.test.ts`, `test/input-controller-escape.test.ts` |
| Pending queue chips | A queued-message chip stays painted from the keystroke until the confirmed queue or the delivered turn retires it, with a bounded expiry for a dispatch that neither queued nor delivered; full snapshot merges never revert a projection field a reliable event updated while the snapshot was in flight. | `test/input-controller-skill-queue.test.ts`, `test/modes/session-port/rpc-session-port-stale-snapshot.test.ts` |
| Barge-in interrupt-submit | `app.message.bargeIn` (Cmd+Return on macOS, Ctrl+Return elsewhere, Alt+Return, Ctrl+X fallback) aborts the in-flight turn with the user-interrupt reason and dispatches the draft through the standard Enter pipeline; Alt+Return belongs to barge-in rather than inserting a newline (Shift+Return still does), while empty drafts, in-flight compaction, and subagent-focused observation stay inert; follow-up keeps Ctrl+Q everywhere and Ctrl+Enter on macOS only. | `test/input-controller-keybindings.test.ts`, `test/keybindings-migration.test.ts`, `test/modes/components/custom-editor.test.ts` |
...
| Mnemopi completion routing | Blank database paths fall back to persistent agent storage; extraction sends localized instructions as a system turn and raw memory text as the user turn across local tiny and smol models, while consolidation keeps its rendered prompt. | `test/mnemopi-config.test.ts`, `test/mnemopi-completion-input.test.ts`, `test/tiny-title-generator.test.ts` |
...
| Persisted session scheduling | Session-bound cron jobs remain crash-safe, lease-claimed, stale-binding guarded, queue-deduplicated, GC'd on terminal retention, and correctly rebound across new/switch/resume/dispose transitions; the agent-facing `cron` tool and the `/cron` management list share the runtime. | `test/scheduling/parser.test.ts`, `test/scheduling/store.test.ts`, `test/scheduling/scheduler.test.ts`, `test/scheduling/runtime.test.ts`, `test/scheduling/stale-delivery.test.ts`, `test/agent-session-scheduling-lifecycle.test.ts` |
