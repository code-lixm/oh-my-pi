...
| Native PDF reading | PDF reads use the `pdf-inspector` native binding, preserve Markdown caching plus line/multi-range/raw selectors, report pages needing OCR, reject legacy PDF image-member paths with browser/OCR guidance, and keep MuPDF WASM/package code out of shipped binaries. | `test/pdf-converter.test.ts`, `test/tools/read-pdf-rendering.test.ts`, `packages/natives/test/native.test.ts` |
